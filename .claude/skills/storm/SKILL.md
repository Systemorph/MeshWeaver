...
| **Fan-out to a dead subscriber** (#2426/#2546) | `[ROUTE] Stream-routed delivery to 'portal/…' / 'node/…' has no live subscriber` at the change-feed rate for hours after a restart/disconnect; senders are per-node owners; "a NACK has no NACK of its own" | Not a retry — the owner's server-side stream is immortal (only an `UnsubscribeRequest` disposes it). The router's refusal is stamped `DeliveryFailure.TargetUnserved`, is delivered to a grain-hosted sender over the grain transport, and the owner EVICTS that subscriber's streams (`Workspace.EvictClientSubscriptions`). Never a negative cache on the delivery path; the LOG is windowed (`DeadTargetRefusalLog`). |
