...
def config_check_source(lambda_src: str) -> str:
    ...
    return "\n".join(text for text, _ in config_check_lines(lambda_src)) + "\n"


# The NodeType's `configuration` lambda has no authored FILE — it is C# inside the node's JSON, on
# `content.configuration`. So it cannot carry a path, and saying so is better than saying nothing:
# its lines are attributed to CONFIGURATION_ORIGIN with the line number WITHIN the lambda, which is
# what an author needs to find it. The generated wrapper around it is the gate's own and carries
# None, exactly like the import block's standard half.
CONFIGURATION_ORIGIN = "<configuration lambda>"


def config_check_lines(lambda_src: str):
    """The ConfigureHub check as (text, origin) pairs — the lambda's own lines attributed to it."""
    out = [
        ("// Generated by compile-check.py — reproduces the ConfigureHub method that", None),
        ("// DynamicMeshNodeAttributeGenerator.GenerateAttributeSource builds around the NodeType's", None),
        ("// `configuration` lambda, so the lambda is type-checked before it can reach a portal.", None),
        ("internal static class __MeshWeaverConfigurationCheck", None),
        ("{", None),
        ("    private static MessageHubConfiguration ConfigureHub(MessageHubConfiguration config)", None),
        ("    {", None),
        ("        var result = config;", None),
        ("        result = result.AddMeshDataSource();", None),
        ("        Func<MessageHubConfiguration, MessageHubConfiguration> userConfig =", None),
    ]
    # Emitted VERBATIM and at column 0 — see the docstring above; only the provenance is added.
    lambda_lines = lambda_src.split("\n")
    for n, line in enumerate(lambda_lines, 1):
        last = n == len(lambda_lines)
        out.append((line + (";" if last else ""), (CONFIGURATION_ORIGIN, n)))
    out += [
        ("        result = userConfig(result);", None),
        ("        return result;", None),
        ("    }", None),
        ("}", None),
    ]
...
def extract_using_statements(lines):
    ...
    for pair in lines:
        ...
        if (key.startswith("using ")
                ...
                and key.endswith(";")
                and "(" not in trimmed):
            # 🚨 The PAIR, not the text: a hoisted directive is MOVED, and a diagnostic on it
            # (CS0246 on a namespace that no longer resolves, CS0105 on a duplicate) lands on the
            # import block — where, without this, `origins` would say None and the gate would print
            # a code with no file. The line moved; its provenance does not have to.
            usings.append(pair)
        else:
            code.append(pair)
    return usings, code
...
def shape_authored_source(pieces):
    ...
    # (text, origin) throughout: the standard block is the platform's own and carries None; an
    # authored directive keeps the file and line it was hoisted out of.
    imports = [(f"using {ns};", None) for ns in IMPLICIT_USINGS]
    already = {text for text, _ in imports}
    for directive, origin in usings:
        trimmed = directive.strip()
        if trimmed and trimmed not in already:
            already.add(trimmed)
            imports.append((trimmed, origin))
    return imports, code
...
def build_unit(cs_files, lambda_src):
    ...
    emit("")
    for directive, origin in imports:
        out.append((directive, origin))
    emit("")
    ...
    if lambda_src:
        ...
        out.extend(config_check_lines(lambda_src))
...
def compile_set(work: Path, cs_files, ref_xml: str, restored: bool, ai_available: bool,
    ...
    needs_external = (not ai_available) and any(names_external(d) for d, _ in imports)
    ...
    for line in out.splitlines():
        ...
        if origin is None:
            ...
            by_key[key] = f"{key}  [{where}:{origin[1]}]"
...
def _self_test() -> int:
    ...
    if [text for text, _ in usings] != [marked]:
        failures.append(f"  a hoisted directive must be emitted VERBATIM, got {usings!r}")
    imports, _ = shape_authored_source([("F", marked)])
    if imports[-1][0] != marked:
        failures.append(f"  the import block must carry the mark verbatim, got {imports[-1][0]!r}")
    # 🚨 …and it keeps the file and line it was MOVED from, so a diagnostic ON a hoisted directive
    # is attributable. Without this the import block is the one region `origins` cannot name — the
    # gap Copilot found on #4727.
    if imports[-1][1] != ("F", 1):
        failures.append(f"  a hoisted directive lost its provenance: {imports[-1][1]!r}")
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            if ([t for t, _ in imports[:len(IMPLICIT_USINGS)]]
                    != [f"using {ns};" for ns in IMPLICIT_USINGS]):
            ...
            with_lambda, lambda_origins, _ = build_unit(files, "config => config")
            # 🚨 The lambda has no authored FILE — it is C# inside the node's JSON — so it is
            # attributed to CONFIGURATION_ORIGIN with its line WITHIN the lambda. Saying "this is
            # the configuration lambda, line N" is the whole difference between a diagnostic an
            # author can find and a code with no location, which is what it had before #4727.
            lines_wl = with_lambda.split("\n")
            lam_line = next((i for i, t in enumerate(lines_wl, 1) if t == "config => config;"), None)
            if lam_line is None or lambda_origins[lam_line - 1] != (CONFIGURATION_ORIGIN, 1):
                failures.append(
                    "  the configuration lambda is not attributed to "
                    f"{CONFIGURATION_ORIGIN}: line={lam_line}, "
                    f"origin={lambda_origins[lam_line - 1] if lam_line else None!r}")
...
def main() -> int:
    ...
    if args.emit_unit is not None:        # [] means 'a source-less set', not 'not asked'
        ...
        json.dump({"imports": [text for text, _ in imports],
