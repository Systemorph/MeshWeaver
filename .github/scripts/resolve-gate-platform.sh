...
if ! set_is_complete; then
  ...
  if [ "$reason" = gone ]; then
    ...
    if [ -n "$CALLER_RUN" ] && [ -n "$oldest" ] && [ "$CALLER_RUN" -lt "$oldest" ]; then
      die "the platform volume at '$VOLUME_ROOT' does not carry ${SET_LABEL}tester digest ${resolved}: core CD #$CALLER_RUN is below the OLDEST set kept (#$oldest). Two histories end here and the volume cannot tell them apart: the set was installed and has since been PURGED, or it was OVERTAKEN before its turn came and was never installed at all. $REFRESH_JOB keeps three things and purges the rest: 'current' (the newest sealed set), a bounded window of the newest sets by install time, and THE SET EACH SUBSCRIBED REPOSITORY'S main LAST PASSED — the one a pull request in that repository resolves. Deliberately no number here: the window's size is the cluster's to set (Systemorph/Memex deployments/aks/ci-runners/ci-platform.yaml), and a number written into this message goes stale the first time it moves, which is exactly how this message came to tell three sessions that the volume keeps the 3 newest while it kept 16. WHAT ACTUALLY FIXES IT turns on one question — is this set the one your repository's main last passed? If YES and your repository is subscribed, the next refresh tick (≤10 min) INSTALLS it back (Systemorph/Memex#329 — rule 2 installs a missing main-passed set, it does not merely decline to purge it); wait for the tick, then re-run the failed jobs and the replayed resolution succeeds unchanged. If your repository is NOT in the refresh's MAIN_PASSED_REPOS, no refresh will ever install this set — add it there; that, and not a re-run, is the fix. 🚨 A RE-RUN IS NOT ITSELF A REMEDY, and believing it is costs hours. 'Re-run failed jobs' cannot re-resolve AT ALL — the job that resolved this set SUCCEEDED, so it is not re-run and its output is replayed verbatim. A FULL re-run does re-resolve, and returns the SAME set for as long as your main has not PASSED on a newer one — which, while your main is red, is forever. Measured 2026-09-17 on MeshWeaver.Plugins#2040: three attempts, one resolution (3.0.0-ci.8820) each time. A re-run helps only AFTER the volume carries the set again. $NOT_THE_DIGEST The volume holds:$(listing). $NO_PULL"
    fi
    if [ -n "$CALLER_RUN" ] && [ -n "$newest" ]; then
      die "the platform volume at '$VOLUME_ROOT' will never carry ${SET_LABEL}tester digest ${resolved}: the refresh has moved PAST it — the volume's newest set is core CD #$newest and its oldest is #${oldest:-?}, so #$CALLER_RUN is INSIDE the kept window and absent, which means it was overtaken (a newer set sealed before the refresh's next tick) and never installed. $REFRESH_JOB keeps three things and purges the rest: 'current' (the newest sealed set), a bounded window of the newest sets by install time, and THE SET EACH SUBSCRIBED REPOSITORY'S main LAST PASSED — the one a pull request in that repository resolves. Deliberately no number here: the window's size is the cluster's to set (Systemorph/Memex deployments/aks/ci-runners/ci-platform.yaml), and a number written into this message goes stale the first time it moves, which is exactly how this message came to tell three sessions that the volume keeps the 3 newest while it kept 16. WHAT ACTUALLY FIXES IT turns on one question — is this set the one your repository's main last passed? If YES and your repository is subscribed, the next refresh tick (≤10 min) INSTALLS it back (Systemorph/Memex#329 — rule 2 installs a missing main-passed set, it does not merely decline to purge it); wait for the tick, then re-run the failed jobs and the replayed resolution succeeds unchanged. If your repository is NOT in the refresh's MAIN_PASSED_REPOS, no refresh will ever install this set — add it there; that, and not a re-run, is the fix. 🚨 A RE-RUN IS NOT ITSELF A REMEDY, and believing it is costs hours. 'Re-run failed jobs' cannot re-resolve AT ALL — the job that resolved this set SUCCEEDED, so it is not re-run and its output is replayed verbatim. A FULL re-run does re-resolve, and returns the SAME set for as long as your main has not PASSED on a newer one — which, while your main is red, is forever. Measured 2026-09-17 on MeshWeaver.Plugins#2040: three attempts, one resolution (3.0.0-ci.8820) each time. A re-run helps only AFTER the volume carries the set again. $NOT_THE_DIGEST The volume holds:$(listing). $NO_PULL"
    fi
    die "the platform volume at '$VOLUME_ROOT' carries no set for tester digest ${resolved}, and this caller passed no --set (or one carrying no core-CD run number — it got '${CALLER_SET:-}'), so which of the three absences this is cannot be told apart here. $REFRESH_JOB runs every 10 minutes. $REFRESH_JOB keeps three things and purges the rest: 'current' (the newest sealed set), a bounded window of the newest sets by install time, and THE SET EACH SUBSCRIBED REPOSITORY'S main LAST PASSED — the one a pull request in that repository resolves. Deliberately no number here: the window's size is the cluster's to set (Systemorph/Memex deployments/aks/ci-runners/ci-platform.yaml), and a number written into this message goes stale the first time it moves, which is exactly how this message came to tell three sessions that the volume keeps the 3 newest while it kept 16. So a set is absent because it is (1) PURGED — outside the window and not any subscribed repository's main-passed set; (2) OVERTAKEN — a newer set sealed before the refresh's next tick, so it was never installed; or (3) ARRIVING — which this shard waits out on its own and would have said so. For (1) and (2) alike, see Systemorph/Memex#329: the remedy is the main-passed rule carrying the set, never a re-run. Pass --set <3.0.0-ci.N> (the resolver's \`set\` output) to have the three named apart. The volume holds:$(listing). $NO_PULL"
  fi
