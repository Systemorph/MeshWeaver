...
🚨 **Operations and diagnostics on a deployment go through the control instance's Hosting API — never the cluster** (maintainer, 2026-09-08: *"all the operations through the memex api"*, *"no direct access of aks"*). An instance is a `Deployments/<name>` record on memex.meshweaver.cloud; a roll, restart, suspend, audit or reconcile is a `Hosting/InstanceAction` node the in-cluster operator executes and reports on. `az aks command invoke` / `kubectl` / Loki-by-curl / a hand `helm-release.yml` dispatch are **break-glass**, and a break-glass write is half an operation (the Hosting manual's "Break glass" section lists the other half). Three reads have no portal answer yet — per-replica image/restarts, a log at time T, "can THIS replica load NodeType X" — and are taken read-only, named as break-glass, never written up as the procedure. Full reference: [OperatingFromThePortal.md](src/MeshWeaver.Documentation/Data/Architecture/OperatingFromThePortal.md).
