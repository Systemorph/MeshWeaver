...
🚨 **A change that spans this repo and a plugin repo lands DELETING-HALF-LAST, and CI now holds you to it.** A pull request whose diff removes a `public` top-level type from `src/` — a departure, a move, or a whole assembly leaving — must declare its counterpart in the PR BODY: `Pairs-with: Systemorph/MeshWeaver.Plugins#904`, or `Pairs-with: none — <reason>` when nothing outside core referenced it. The `Cross-repo pair (public surface)` gate resolves it and stays red while the counterpart is open, draft, or merged into anything but its repo's default branch. Ordinary work never meets it (measured: `main~25 → main` removes ZERO public types). Full reference: [CrossRepoPairGate.md](src/MeshWeaver.Documentation/Data/Architecture/CrossRepoPairGate.md) · [/ci](.claude/skills/ci/SKILL.md).
