...
🚨 **A change that spans this repo and a plugin repo lands DELETING-HALF-LAST, and CI now holds you to it.** A pull request whose diff removes a `public` top-level type from `src/` — a departure, a move, or a whole assembly leaving — **or a public MEMBER of a type that stays** (#3137 deleted two fields and reddened every Plugins PR for three hours) must declare its counterpart in the PR BODY: `Pairs-with: Systemorph/MeshWeaver.Plugins#904`, or `Pairs-with: none — <reason>` when nothing outside core referenced it (a reason citing a live-mesh sweep must quote `searched: true`; `searched: false` is refused). The `Cross-repo pair (public surface)` gate resolves it and stays red while the counterpart is open, draft, or merged into anything but its repo's default branch. Ordinary work never meets it (measured: `main~25 → main` removes ZERO public types and two members, both #3137). **And NOTHING runs the dependent's suites for you.** The `Dependent suites (MeshWeaver.Plugins)` check this line used to name was withdrawn the day it landed (2026-09-03: *"none of the top-level repos should have any dependency to anyone else"*), so a core PR carries no context a plugin repo can turn green — and an absent required context reads as green. An ADDITIVE or BEHAVIOURAL break — the same signature answering differently — is caught only when the dependent next COMPILES against core, and MeshWeaver.Plugins **pins** core (`MW_PLATFORM_REF`), so that is its next pin bump, not the release event. #3276 sat 48 commits that way. Changing what a shipped surface DOES? Say so on the PR and expect the dependent's pin bump to be where it lands. Full reference: [CrossRepoPairGate.md](src/MeshWeaver.Documentation/Data/Architecture/CrossRepoPairGate.md) · [/ci](.claude/skills/ci/SKILL.md).
