...
🚨 **This paragraph used to say a new key had a SECOND home in `clients/react/src/i18n/`, with a byte-identical mirror and a drift guard. That is gone.** `044a85618` ("The GUI leaves the platform: the view packs and the web clients", 2026-08-25) moved the web clients to MeshWeaver.Plugins; `clients/` now holds only `python/` and `voice-gateway/`, and **all six** guards the old text and the `Clients` workflow named — `i18n/localize.test.ts`, `render/parity.test.ts`, `ciTrigger.test.ts`, `grpc-web/src/wireContract.test.ts`, `grpc-web/src/restContract.test.ts`, `portal-next/e2e` — no longer exist in this repo.

**Do not go looking for the mirror, and do not recreate one here.** If a JS client still needs a synchronous catalog copy, that guard belongs where the client now lives. Recorded rather than deleted because the instruction was live in this file for two days after the move, and an instruction pointing at a deleted directory is the *"prose asserts a guard that does not exist"* failure this file warns about elsewhere — the reader cannot tell a stale rule from a rule they have failed to follow.
