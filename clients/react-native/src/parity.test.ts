...
const EXTERNALLY_PACKED_CONTROLS = [
  ...
  // controls, extracted from the core registry onto the view-pack seam; the control records stay
  // in MeshWeaver.Layout. MeshNodePicker moved with them (to MeshWeaver.Blazor.Graph,
...
describe("React Native ↔ Blazor control parity", () => {
  it("scrapes a plausible vocabulary out of BlazorViewRegistry.cs", () => {
    ...
    // assertions below would vacuously pass on an empty list. (Was >40 before the entity form
    // controls — 11 arms — left the switch for the EntityViews/Graph packs; they are pinned under
    // EXTERNALLY_PACKED_CONTROLS above, so coverage moved rather than shrank.)
