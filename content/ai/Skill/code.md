...
Before you write any handler, layout area, click action, service method, or Blazor view, you must internalise five documents. Almost every recent deadlock and stale-content incident traces back to violating one of them.
...
5. **[Localization](@/Doc/Architecture/Localization)** — **every string a user reads is localized, as you write it.** The portal ships English + German, so a hard-coded UI string is a bug: it renders English for every viewer regardless of their language. Text with no declaration to hang off (Blazor markup, inline `Controls.*` literals, toasts) gets a key in BOTH `strings.en.json` and `strings.de.json` and is read via `host.Localize("key")` (layout areas) or `Access.Localize("key")` (Blazor, `@inject AccessService Access`); text ON a declaration (property label, node-type name, enum member) carries `[Translation("de", "…")]` next to its `[Description]` so the two cannot drift. Covers labels, buttons, tooltips, `aria-label`s, placeholders, page titles, empty states, validation messages and dialog copy. Prefer wording that needs no translation at all — a language-neutral glyph (**➕**, ✏️, 🗑️) plus a translated tooltip beats a translated label. 🚨 NEVER resolve from `CultureInfo.CurrentUICulture` — a layout-area render hops the hub scheduler and an ambient AsyncLocal culture does not survive it, so one viewer's UI would pick up another's language; resolution is always explicit off `AccessContext.Locale`. 🚨 Do NOT translate LLM tool-parameter `[Description]`s (model-facing — it degrades tool-calling) or wire identifiers (`nodeType:Thread` in help text, action keys, icon names).
