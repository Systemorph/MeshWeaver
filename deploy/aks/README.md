...
|---|---|---|
| a `readinessProbe` exists **on `/ready`** | that probe is the ONLY reader of the gate (policy `bake-gate-readiness-only`); readiness anywhere else ignores it entirely. The `startupProbe` must NOT be its reader — a failing startup probe kills the container | `kubectl -n <ns> get deploy memex-portal-deployment -o jsonpath='{.spec.template.spec.containers[0].readinessProbe}'` |
