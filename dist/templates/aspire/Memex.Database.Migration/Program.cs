...
// ── Data repair v5: Ensure all users have Admin self-assignment and rebuild permissions ──
// Fixes missing AccessAssignment nodes for users who were onboarded before UserScopeGrantHandler,
// and rebuilds user_effective_permissions to include Thread permission (added to Admin/Editor roles).
if (currentVersion < 5)
{
    logger.LogInformation("Running repair v5: Ensure user self-assignments and rebuild permissions...");
    await using (var cmd = dataSource.CreateCommand("""
        DO $$
        DECLARE
            user_rec RECORD;
