...
/// <summary>
/// Reads the Admin node (nodeType "Platform") directly from graph storage at startup,
...
public static class AdminStartupReader
{
    ...
    /// <summary>
    /// Reads PlatformSettings from the Admin node at startup.
    /// Supports FileSystem (direct file read) and PostgreSql (ADO.NET query).
    /// Returns null if the node doesn't exist (fresh install).
    /// </summary>
    public static PlatformSettings? ReadPlatformSettings(IConfiguration config, ILogger? logger = null)
    {
        ...
        {
            logger?.LogWarning(ex, "Failed to read Admin node from {StorageType} storage. Falling back to config.", storageType);
            return null;
        }
    }
    ...
    public static List<ExternalProviderConfig> ResolveProviders(
        PlatformSettings settings, string? keyVaultUri, ILogger? logger = null)
    {
    ...
    private static PlatformSettings? ReadFromFileSystem(IConfiguration config, ILogger? logger)
    {
        ...
        var filePath = Path.Combine(basePath, "Admin.json");
        if (!File.Exists(filePath))
        {
            logger?.LogDebug("Admin.json not found at {Path}. Fresh install assumed.", filePath);
            return null;
        }
        ...
        {
            return JsonSerializer.Deserialize<PlatformSettings>(
    ...
    private static PlatformSettings? ReadFromPostgreSql(IConfiguration config, ILogger? logger)
    {
        var connectionString = config.GetConnectionString("meshweaver");
        if (string.IsNullOrWhiteSpace(connectionString))
        {
            logger?.LogDebug("No 'meshweaver' connection string found. Cannot read Admin node from PostgreSql.");
            return null;
        }
        ...
        // Admin node: namespace = '' (root), id = 'Admin'
        ...
        {
            logger?.LogDebug("Admin node not found in PostgreSql. Fresh install assumed.");
            return null;
        }
        ...
        return JsonSerializer.Deserialize<PlatformSettings>(contentJson, JsonOptions);
    }
}
