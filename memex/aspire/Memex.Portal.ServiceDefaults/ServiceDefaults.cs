...
#pragma warning restore EXTEXP0001
    ...
    /// required-modules check (policy <c>required-modules-readiness-only</c>). A missing required
    /// module is a property of what the pod's shelf was given, and when the registry cannot serve
    /// it, it is missing for every image — so on the startup probe it would kill the previous
    /// image's restarted pods too. As a roll gate it stalls the roll and keeps the pod out of the
    /// Service instead. The other checks a portal registers stay on the startup probe deliberately
