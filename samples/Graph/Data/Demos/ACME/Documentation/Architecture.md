...
The ACME sample demonstrates a modular approach where the Todo functionality is encapsulated in a reusable NodeType (`Demos/ACME/Project/Todo`) with clear message-based interfaces, making it easy to share across multiple projects within the organization.
...
| Todo NodeType | `Demos/ACME/Project/Todo` |
...
The Todo NodeType (`Demos/ACME/Project/Todo`) is configured with:

```json
{
  "id": "Todo",
  "namespace": "Demos/ACME/Project",
  "name": "Task",
  "nodeType": "NodeType",
  "content": {
...
public static IObservable<UiControl?> TodaysFocus(LayoutAreaHost host, RenderingContext _)
{
    var nodes = meshQuery.ObserveQuery<MeshNode>(
        $"path:{hubPath}/Todo nodeType:Demos/ACME/Project/Todo state:Active scope:subtree");
