// <meshweaver>
// Id: Testing/CompilerPipeline/ContentKeyReevaluationTest
// DisplayName: Testing/CompilerPipeline/ContentKeyReevaluationTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public class ContentKeyReevaluationTest
{
    ...
    /// </summary>
    [MeshTheory]
    [MeshInlineData(null, Digest)]          // the input could not be regenerated
    [MeshInlineData(Digest, null)]          // the record predates the key (an adopted prebuilt, a cache hit)
    [MeshInlineData(null, null)]            // neither
    ...
    [MeshInlineData(true, false, ReevaluationVerdict.CarryForward, true,
        NodeTypeCompilationHelpers.StaleBuildAction.RestampDependencyRecord)]
    // …but only when the production predicate agrees it is now usable.
    [MeshInlineData(true, false, ReevaluationVerdict.CarryForward, false,
        NodeTypeCompilationHelpers.StaleBuildAction.Rebuild)]
    [MeshInlineData(true, false, ReevaluationVerdict.Rebuild, false,
        NodeTypeCompilationHelpers.StaleBuildAction.Rebuild)]
    [MeshInlineData(true, false, ReevaluationVerdict.Inconclusive, false,
        NodeTypeCompilationHelpers.StaleBuildAction.Rebuild)]
    // framework MOVED, store MISS — compile, exactly as before the lane existed.
    [MeshInlineData(false, false, ReevaluationVerdict.CarryForward, true,
        NodeTypeCompilationHelpers.StaleBuildAction.Rebuild)]
    // framework MOVED, store HIT — the branch that used to skip unconditionally. A moved input is
    // now a NEW invalidation…
    [MeshInlineData(false, true, ReevaluationVerdict.Rebuild, false,
        NodeTypeCompilationHelpers.StaleBuildAction.Rebuild)]
    // …and everything else keeps the pre-lane behaviour: skip, and change nothing.
    [MeshInlineData(false, true, ReevaluationVerdict.CarryForward, true,
        NodeTypeCompilationHelpers.StaleBuildAction.Skip)]
    [MeshInlineData(false, true, ReevaluationVerdict.Inconclusive, false,
        NodeTypeCompilationHelpers.StaleBuildAction.Skip)]
