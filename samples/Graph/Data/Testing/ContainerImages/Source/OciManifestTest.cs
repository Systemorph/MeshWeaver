// <meshweaver>
// Id: Testing/ContainerImages/OciManifestTest
// DisplayName: Testing/ContainerImages/OciManifestTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public class OciManifestTest
{
    ...
    [MeshInlineData("""{"schemaVersion":1,"fsLayers":[{"blobSum":"sha256:ab"}]}""")]
    ...
    [MeshInlineData("""{"mediaType":"application/vnd.oci.image.index.v1+json","manifests":[]}""")]
    // Layers without a config: not a manifest shape.
    [MeshInlineData("""{"layers":[{"digest":"sha256:ab","size":1}]}""")]
    // A config that names no digest cannot anchor a closure.
    [MeshInlineData("""{"config":{"size":1},"layers":[]}""")]
    // Not an object.
    [MeshInlineData("[]")]
    [MeshInlineData("\"a string\"")]
    // Not JSON at all — a truncated body, or an error page an intermediary substituted.
    [MeshInlineData("<html>502</html>")]
    [MeshInlineData("")]
