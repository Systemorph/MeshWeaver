// <meshweaver>
// Id: Testing/Data/ReadDuringDisposalWindowTest
// DisplayName: Testing/Data/ReadDuringDisposalWindowTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
/// </summary>
public class ReadDuringDisposalWindowTest(MeshTestContext context) : InMeshTestBase(output)
{
    ...
    public async Task ReadThatFaultsOnTeardown_IsNackedShuttingDown_NotAnsweredAsAbsent()
    {
        ...
        await owner!.Started.WaitAsync(TimeSpan.FromSeconds(30), CancellationToken.None);
    ...
    public async Task AWarmedReferenceIsAnsweredFromTheLiveMirror()
    {
        ...
        var owner = host.GetHostedHub(
            ...
            c => c.WithTypes(typeof(ParkRequest), typeof(Item))
                ...
                .WithPostingIdentity(PostingIdentity.System));
        owner.Should().NotBeNull();
        await owner!.Started.WaitAsync(TestTimeouts.Convergence, CancellationToken.None);
