// <meshweaver>
// Id: Testing/Documentation/DocumentationEmbedIntegrityTest
// DisplayName: Testing/Documentation/DocumentationEmbedIntegrityTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
/// <c>&lt;code&gt;</c> is not an embed) in THIS repository's embedded documentation targets a node
/// in a partition that SHIPS WITH THE DOCS.
///
/// <para>The docs are served on their own (the <c>Doc</c> partition, plus <c>Agent</c>/<c>Skill</c>
/// from the AI engine); on a real portal the sample partitions (<c>PythonDemo</c>, <c>Northwind</c>,
/// <c>ACME</c>, …) are NOT loaded. A doc page that <c>@@</c>-embeds a node from a non-shipped
/// partition renders the ugly "No renderer is registered for area <c>{firstSegment}</c> on hub
/// <c>{docHub}</c>" — the exact failure on <c>Doc/Architecture/PythonCodeNodes</c> (the raw path
/// <c>@@PythonDemo/SampleStatistics</c> did not resolve, so the client fell back to treating
/// <c>PythonDemo</c> as an area on the doc's own hub). Example nodes must therefore ship IN the Doc
/// partition (as <c>Cession</c> / <c>SocialMedia</c> do), not be embedded from a sample partition.</para>
///
/// <para>🚨 WHY IT LIVES HERE. Its subject is this repository's own doc tree, and until now the only
/// thing pinning it was <c>MeshWeaver.AI.Test</c> in the PRIVATE sibling <c>MeshWeaver.Plugins</c> —
/// so a core pull request that broke a doc embed went green here and turned a DIFFERENT repository
/// red on a change it did not make. A gate belongs in the repository that owns what it measures.</para>
///
/// <para>Scope, deliberately: the shippable set seeds <c>Agent</c> and <c>Skill</c> as literals
/// (they ship alongside the docs, from <c>MeshWeaver.AI</c>) but enumerates only the <c>Doc</c>
/// partition's own resources, which are all this assembly can see. The union — including whether a
/// specific <c>@@Agent/…</c> or <c>@@Skill/…</c> target exists — stays pinned by the copy in
/// MeshWeaver.Plugins, the repo that holds both assemblies. That is the correct direction.</para>
///
/// <para>A shippable partition is <c>Doc</c>/<c>Agent</c>/<c>Skill</c>, the first segment of any node
/// shipped in the Doc data (json namespaces), or the id of a <c>NodeType</c> node shipped there (a
...
public class DocumentationEmbedIntegrityTest
{
    ...
    public void AllLiveDocEmbeds_TargetPartitionsThatShipWithTheDocs()
    {
        ...
        var markdown = LoadResources(docAssembly, DocResourcePrefix, "Doc", ".md").ToImmutableArray();

        markdown.Should().NotBeEmpty(
            "the Doc partition's markdown must be embedded in MeshWeaver.Documentation — an empty " +
            "resource set would make this gate pass having checked nothing");

        var shippablePartitions = BuildShippablePartitions(docAssembly);
    ...
    [MeshInlineData("Doc/Architecture/PythonCodeNodes", "@@SampleStatistics", "Doc/Architecture/PythonCodeNodes/SampleStatistics")]
    // sibling hyperlink (one ..)
    [MeshInlineData("Doc/Architecture/PythonCodeNodes", "@../ForeignLanguageIntegration", "/Doc/Architecture/ForeignLanguageIntegration")]
    // cross-area hyperlink (two .. then down)
    [MeshInlineData("Doc/Architecture/PythonCodeNodes", "@../../DataMesh/PythonPandasNode", "/Doc/DataMesh/PythonPandasNode")]
    [MeshInlineData("Doc/Architecture/AccessContextPropagation", "@../AsynchronousCalls", "/Doc/Architecture/AsynchronousCalls")]
    [MeshInlineData("Doc/Architecture/AccessContextPropagation", "@../../GUI/DataBinding", "/Doc/GUI/DataBinding")]
    ...
    /// </summary>
    [MeshTheory]
    [MeshInlineData("Doc/Architecture/PythonCodeNodes")]
    [MeshInlineData("Doc/Architecture/AccessContextPropagation")]
    ...
    /// <summary>Every node PATH shipped in the Doc data — markdown pages (from the resource name) and
    /// <c>.json</c> nodes (from their declared namespace + id), plus the <c>Agent</c>/<c>Skill</c>
    /// partition roots that ship alongside from MeshWeaver.AI.</summary>
    private static ImmutableHashSet<string> BuildShippedNodePaths()
    {
        ...
        foreach (var (path, _) in LoadResources(docAssembly, DocResourcePrefix, "Doc", ".md"))
            set.Add(path);
        foreach (var (_, json) in LoadResources(docAssembly, DocResourcePrefix, partition: null, ".json"))
        {
            try
            {
    ...
    /// Doc/Agent/Skill, the partition of every node shipped in the Doc data, and (because a NodeType's
    /// instances address under its own name) the id of every shipped <c>NodeType</c> node.
    /// </summary>
    private static ImmutableHashSet<string> BuildShippablePartitions(Assembly docAssembly)
    {
        var set = ImmutableHashSet.CreateBuilder<string>(StringComparer.OrdinalIgnoreCase);
        set.Add("Doc");
        set.Add("Agent");
