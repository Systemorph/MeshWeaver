// <meshweaver>
// Id: Testing/Documentation/MainRunsAreNeverCancelledGuard
// DisplayName: Testing/Documentation/MainRunsAreNeverCancelledGuard — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public class MainRunsAreNeverCancelledGuard
{
    ...
    [MeshInlineData("false", "main-cd-${{ github.event_name == 'schedule' && 'reconcile' || github.ref }}", "")]
    // The flag flipped, as a literal and as an expression that spares only the reconcile.
    [MeshInlineData("true", "main-cd-${{ github.event_name == 'schedule' && 'reconcile' || github.ref }}", "run in flight")]
    [MeshInlineData("${{ github.event_name != 'schedule' }}", "main-cd-${{ github.event_name == 'schedule' && 'reconcile' || github.ref }}", "run in flight")]
    // The reconcile folded back into the push lane's group (#2490's shape).
    [MeshInlineData("false", "main-cd-${{ github.ref }}", "reconcile")]
    // A per-run or per-commit group: serializes nothing, every delivery publishes concurrently.
    [MeshInlineData("false", "main-cd-${{ github.run_id }}", "its own group")]
    [MeshInlineData("false", "main-cd-${{ github.sha }}", "its own group")]
    ...
    [MeshInlineData("${{ github.event_name != 'workflow_dispatch' && github.ref != 'refs/heads/main' }}", "push", "refs/heads/main", false)]
    [MeshInlineData("${{ github.event_name != 'workflow_dispatch' && github.ref != 'refs/heads/main' }}", "pull_request", "refs/pull/1/merge", true)]
    // The pre-fix expression, and a bare literal: both cancel on main.
    [MeshInlineData("${{ github.event_name != 'workflow_dispatch' }}", "push", "refs/heads/main", true)]
    [MeshInlineData("true", "push", "refs/heads/main", true)]
    // 🚨 Copilot's counterexample on this PR: mentions refs/heads/main, still cancels there.
    [MeshInlineData("${{ github.ref == 'refs/heads/main' }}", "push", "refs/heads/main", true)]
    // A disjunct is not a conjunct — `||` re-admits main whenever the other side is true.
    [MeshInlineData("${{ github.event_name != 'workflow_dispatch' || github.ref != 'refs/heads/main' }}", "push", "refs/heads/main", true)]
    // Negation and parentheses, the two shapes a rewrite reaches for.
    [MeshInlineData("${{ !(github.ref == 'refs/heads/main') }}", "push", "refs/heads/main", false)]
    [MeshInlineData("${{ !(github.ref == 'refs/heads/main') }}", "pull_request", "refs/pull/1/merge", true)]
    [MeshInlineData("false", "pull_request", "refs/pull/1/merge", false)]
