// <meshweaver>
// Id: Testing/Documentation/TestTimeoutLiteralRatchetGuard
// DisplayName: Testing/Documentation/TestTimeoutLiteralRatchetGuard — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
/// <c>[MeshFact(TimeoutSeconds = 30)]</c> — is a guess about how fast a machine is, written where it can
...
public class TestTimeoutLiteralRatchetGuard
{
    ...
    public void TheHandWrittenTimeoutCountOnlyEverGoesDown()
    {
        ...
        Assert.True(total <= Baseline,
            ...
            + "[MeshFact(Timeout = …)] instead of writing a literal.\n\n"
    ...
    public void TheScannerSeesWhatItClaimsTo()
    {
        ...
        try
        {
            ...
            File.WriteAllText(Path.Combine(t, "FactTimeout.cs"),
                "class C { [MeshFact(TimeoutSeconds = 30)] public void M() { } }");
            File.WriteAllText(Path.Combine(t, "Underscored.cs"),
                "class D { [MeshFact(TimeoutSeconds = 1)] public void M() { } }");
            ...
            Assert.True(found.ContainsKey("FactTimeout.cs"), "[MeshFact(TimeoutSeconds = 30)] must be found");
