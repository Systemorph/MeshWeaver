// <meshweaver>
// Id: Testing/Graph/NuGetAssemblyResolverTest
// DisplayName: Testing/Graph/NuGetAssemblyResolverTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public class NuGetAssemblyResolverTest
{
    ...
    {
        await SkipUnlessNuGetOrgReachable(CancellationToken.None);
    ...
    public async Task Resolve_TwiceWithSameInputs_HitsCache()
    {
        ...
        var first = await resolver.ResolveAsync(refs, targetFramework: null, CancellationToken.None);
        var second = await resolver.ResolveAsync(refs, targetFramework: null, CancellationToken.None);
