// <meshweaver>
// Id: Testing/Graph/ServedBuildIsVerifiableTest
// DisplayName: Testing/Graph/ServedBuildIsVerifiableTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
/// publication.</para>
/// </summary>
public class ServedBuildIsVerifiableTest(MeshTestContext context) : InMeshTestBase(output)
{
    ...
    [MeshInlineData(PublishedMvid, null)]
    [MeshInlineData(null, BoundMvid)]
    [MeshInlineData("", BoundMvid)]
    [MeshInlineData(PublishedMvid, "")]
    [MeshInlineData(PublishedMvid, PublishedMvid)]
    ...
    /// </summary>
    [MeshTheory]
    [MeshInlineData(PublishedMvid, BoundMvid, 0, true)]   // mismatch, budget left → refuse
    [MeshInlineData(PublishedMvid, BoundMvid, 1, false)]  // budget exhausted → bind + banner (degraded beats parked)
    [MeshInlineData(PublishedMvid, PublishedMvid, 0, false)] // identical bytes → bind
    [MeshInlineData(null, BoundMvid, 0, false)]           // legacy node, no published MVID → no evidence → bind
    [MeshInlineData(PublishedMvid, null, 0, false)]       // unreadable local file → no evidence → bind
