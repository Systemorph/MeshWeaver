// <meshweaver>
// Id: Testing/Graph/TestTimeoutsTest
// DisplayName: Testing/Graph/TestTimeoutsTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
{
    /// <summary>
    /// 🚨 THE INVARIANT. 24 files carry `[MeshFact(TimeoutSeconds = 30)]` AND a 30 s internal wait, so
    ...
    public void TheOuterBoundAlwaysExceedsTheInner(string? factor)
    {
        ...
        Assert.True(
            ...
            $"the [MeshFact(Timeout)] value ({TestTimeouts.TestMilliseconds} ms) must exceed the "
