// <meshweaver>
// Id: Testing/Hosting/ActivityTrackerQuiesceTest
// DisplayName: Testing/Hosting/ActivityTrackerQuiesceTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public class ActivityTrackerQuiesceTest
{
    ...
    public async Task Track_StillCounts_AndWhenIdleFiresOnRelease()
    {
        ...
        await tracker.InFlightChanges.Where(running => running == 1).FirstAsync()
            .Timeout(TimeSpan.FromSeconds(5)).Await(CancellationToken.None);

        var idle = tracker.WhenIdle.FirstAsync().Timeout(TimeSpan.FromSeconds(5)).Await(CancellationToken.None);
        await Task.Yield();
