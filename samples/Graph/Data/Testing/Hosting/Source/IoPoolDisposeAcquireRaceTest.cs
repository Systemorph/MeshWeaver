// <meshweaver>
// Id: Testing/Hosting/IoPoolDisposeAcquireRaceTest
// DisplayName: Testing/Hosting/IoPoolDisposeAcquireRaceTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public class IoPoolDisposeAcquireRaceTest
{
    ...
    public async Task Invoke_built_before_disposal_and_subscribed_after_is_cancelled_not_ObjectDisposed()
    {
        ...
        var fault = await Record.ExceptionAsync(
            () => cold.FirstAsync().Await(CancellationToken.None));
    ...
    public async Task Disposing_while_a_leaf_is_taking_the_gate_never_surfaces_ObjectDisposed()
    {
        for (var attempt = 0; attempt < 40; attempt++)
        {
            ...
            var leaked = await disposed.WaitAsync(Timeout10, CancellationToken.None);
            leaked.Should().Be(0);
    ...
    public async Task Disposal_completes_even_with_a_live_SubscribeThroughPool_subscription()
    {
        ...
        await terminated.Task.WaitAsync(Timeout10, CancellationToken.None);
        var leaked = await disposed.WaitAsync(Timeout10, CancellationToken.None);
