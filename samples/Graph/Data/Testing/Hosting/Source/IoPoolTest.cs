// <meshweaver>
// Id: Testing/Hosting/IoPoolTest
// DisplayName: Testing/Hosting/IoPoolTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public class IoPoolTest
{
    ...
    public async Task Dispose_cancels_without_blocking_and_reports_through_Disposed()
    {
        ...
        var residual = await pool.Disposed.Timeout(Timeout10).FirstAsync().Await(CancellationToken.None);
    ...
    public async Task Disposed_waits_for_the_last_leaf_and_replays_to_a_late_subscriber()
    {
        ...
        try
        {
            ...
            await pool.Disposed.Timeout(Timeout10).FirstAsync().Await(CancellationToken.None);
    ...
    public async Task Registry_Disposed_reports_only_after_every_pool_has_finished()
    {
        ...
        try
        {
            ...
            var total = await registry.Disposed.Timeout(Timeout10).FirstAsync().Await(CancellationToken.None);
    ...
    public async Task A_pool_requested_after_disposal_is_refused_not_created_live()
    {
        ...
        registry.Dispose();
        await registry.Disposed.Timeout(Timeout10).FirstAsync().Await(CancellationToken.None);
        ...
        var act = async () => await late.Invoke(_ => { ran = true; return Task.FromResult(1); })
            .FirstAsync().Await(CancellationToken.None);
    ...
    public async Task InvokeBlocking_caps_concurrency_and_runs_off_thread()
    {
        ...
        try
        {
            ...
            Volatile.Write(ref release, 1);
            await Task.WhenAll(tasks).WaitAsync(Timeout10, CancellationToken.None);
    ...
    public async Task Drain_joins_InvokeBlocking_leaves_too()
    {
        ...
        try
        {
            ...
            // (Copilot review, #1334.)
            var drain = Task.Run(pool.Drain, CancellationToken.None);
            ...
            var raced = await Task.WhenAny(drain, Task.Delay(300, CancellationToken.None));
            ...
            var residual = await drain.WaitAsync(Timeout5, CancellationToken.None);
    ...
    public async Task Drain_cancels_in_flight_leaves_and_joins_synchronously()
    {
        ...
        Func<Task> afterDrain = () =>
            pool.Invoke(_ => Task.FromResult(7)).Await(CancellationToken.None);
    ...
    public async Task Drain_terminates_a_SubscribeThroughPool_leg_it_cancels()
    {
        ...
        try
        {
            ...
            var drainDone = Task.Run(pool.Drain, CancellationToken.None);
            ...
            await drainDone.WaitAsync(Timeout10, CancellationToken.None);
    ...
    public async Task Drain_terminates_a_SubscribeThroughPool_leg_that_already_subscribed()
    {
        ...
        var residual = await drainDone.WaitAsync(Timeout10, CancellationToken.None);
        residual.Should().Be(0,
    ...
    public async Task Drain_runsPooledSubscriptionTeardown_offTheCallersThread()
    {
        ...
        try
        {
            ...
            var residual = await drain.WaitAsync(Timeout10, CancellationToken.None);
    ...
    {
        var pool = new IoPool(2);
        await pool.Invoke(_ => Task.FromResult(1)).FirstAsync().Await(CancellationToken.None);
