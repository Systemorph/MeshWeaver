// <meshweaver>
// Id: Testing/HostingOrleans/NeverActiveHolderIsNotAliveTest
// DisplayName: Testing/HostingOrleans/NeverActiveHolderIsNotAliveTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public class NeverActiveHolderIsNotAliveTest
{
    ...
    [MeshInlineData(SiloStatus.Created, ClusterMemberState.Unknown)]
    [MeshInlineData(SiloStatus.Joining, ClusterMemberState.Unknown)]
    // Reached Active and still executing — including while it drains. Anything it holds, it holds.
    [MeshInlineData(SiloStatus.Active, ClusterMemberState.Alive)]
    [MeshInlineData(SiloStatus.ShuttingDown, ClusterMemberState.Alive)]
    [MeshInlineData(SiloStatus.Stopping, ClusterMemberState.Alive)]
    // The ONE positive departure verdict.
    [MeshInlineData(SiloStatus.Dead, ClusterMemberState.Gone)]
    // Absence is not death (see the class remarks on OrleansClusterMembership).
    [MeshInlineData(SiloStatus.None, ClusterMemberState.Unknown)]
