// <meshweaver>
// Id: Testing/HostingOrleans/OrderedRouteDispatcherTest
// DisplayName: Testing/HostingOrleans/OrderedRouteDispatcherTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public class OrderedRouteDispatcherTest(MeshTestContext context) : InMeshTestBase(context)
{
    ...
    public async Task SameDestination_SubscribesLegsInOrder_OneAtATime()
    {
        ...
        // Leg 0 starts…
        (await starts.Take(1).Timeout(TimeSpan.FromSeconds(10)).Await()).Should().Be(0);
        ...
        await Assert.ThrowsAsync<TimeoutException>(() =>
            starts.Skip(1).Take(1).Timeout(TimeSpan.FromSeconds(1)).Await());

        for (var i = 0; i < gates.Length - 1; i++)
        {
            gates[i].OnNext(Unit.Default);
            gates[i].OnCompleted();
            (await starts.Skip(i + 1).Take(1).Timeout(TimeSpan.FromSeconds(10)).Await()).Should().Be(i + 1,
                "each leg is subscribed only after the one ahead of it has completed, in arrival order");
        }

        gates[^1].OnCompleted();
        var order = await starts.Take(3).ToList().Timeout(TimeSpan.FromSeconds(10)).Await();
    ...
    public async Task DifferentDestinations_NeverWaitOnEachOther()
    {
        ...
        var seen = await starts.Take(2).ToList().Timeout(TimeSpan.FromSeconds(10)).Await();
    ...
    public async Task DrainedDestination_IsReleased_SoTheSiloHoldsNoPerAddressState()
    {
        ...
        await starts.Take(1).Timeout(TimeSpan.FromSeconds(10)).Await();
        dispatcher.ActiveDestinations.Should().Be(1);

        var drained = completed.Take(1).Timeout(TimeSpan.FromSeconds(10)).Await();
        gate.OnCompleted();
        await drained;
        ...
        var released = await Observable.Interval(TimeSpan.FromMilliseconds(20)).StartWith(0L)
    ...
    public async Task UnorderedPoolDispatch_StartsEveryLegConcurrently()
    {
        ...
        var seen = await starts.Take(3).ToList().Timeout(TimeSpan.FromSeconds(10)).Await();
