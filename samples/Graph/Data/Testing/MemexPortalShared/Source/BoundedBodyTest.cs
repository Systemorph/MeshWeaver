// <meshweaver>
// Id: Testing/MemexPortalShared/BoundedBodyTest
// DisplayName: Testing/MemexPortalShared/BoundedBodyTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public class BoundedBodyTest
{
    ...
    public async Task ReadBytes_is_byte_exact_for_input_that_is_not_valid_utf8()
    {
        ...
        var got = await BoundedBody.ReadBytesAsync(new MemoryStream(raw), 1024, CancellationToken.None);
        Assert.Equal(raw, got);

        // And the contrast that makes the point: the string form cannot round-trip these bytes.
        var viaString = await BoundedBody.ReadAsync(new MemoryStream(raw), 1024, CancellationToken.None);
    ...
    public async Task ReadBytes_over_the_cap_is_refused()
    {
        ...
        var got = await BoundedBody.ReadBytesAsync(new MemoryStream(over), 64, CancellationToken.None);
        Assert.Null(got);
    }
    ...
    public async Task ReadBytes_at_exactly_the_cap_is_returned()
    {
        ...
        var got = await BoundedBody.ReadBytesAsync(new MemoryStream(exact), 64, CancellationToken.None);
