// <meshweaver>
// Id: Testing/MemexPortalShared/NoOpEmailSenderRefusalTest
// DisplayName: Testing/MemexPortalShared/NoOpEmailSenderRefusalTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public class NoOpEmailSenderRefusalTest
{
    ...
    [MeshTheory]
    [MeshInlineData(false, false, false)] // disabled + no-op: the intended configuration
    [MeshInlineData(false, true, false)]  // disabled + a real sender: nothing to refuse
    [MeshInlineData(true, true, false)]   // enabled + a real sender: the working install
    [MeshInlineData(true, false, true)]   // 🚨 enabled + no-op: the refused configuration
