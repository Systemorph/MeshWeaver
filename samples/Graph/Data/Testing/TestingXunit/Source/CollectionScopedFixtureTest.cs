// <meshweaver>
// Id: Testing/TestingXunit/CollectionScopedFixtureTest
// DisplayName: Testing/TestingXunit/CollectionScopedFixtureTest — migrated from xunit (convert-xunit-to-inmesh.py)
// </meshweaver>
#nullable enable
...
public abstract class CollectionScopedFixtureTestBase
{
    ...
    private protected async Task<CountedResource> AssertSharedAndRecord()
    {
        ...
        // and a [MeshTheory] row is a case. Before the execution host, each of these paid its own.
...
public class AlphaCollectionTest : CollectionScopedFixtureTestBase
{
    ...
    [MeshTheory]
    [MeshInlineData("first", 1)]
    [MeshInlineData("second", 2)]
    [MeshInlineData("third", 3)]
    [MeshInlineData("fourth", 4)]
    [MeshInlineData("fifth", 5)]
    ...
    [MeshFact(Skip = "Skip= is xunit's and must survive the host substitution — this must never run.")]
    public void ASkippedCase_DoesNotRun() =>
        Assert.Fail("a skipped case executed — Skip= did not survive the custom framework");

    [MeshTheory]
    [MeshInlineData(1)]
    [MeshInlineData(2, Skip = "one row skipped, the other must still run")]
