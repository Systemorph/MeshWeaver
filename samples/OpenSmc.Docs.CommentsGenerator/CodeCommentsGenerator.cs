...
namespace OpenSmc.Docs.CodeComments;
...
public class CodeCommentsGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        IncrementalValueProvider<ImmutableArray<AdditionalText>> files = context
            .AdditionalTextsProvider.Where(at => at.Path.EndsWith(".xml"))
            .Collect();
    ...
    //TODO Roland Bürgi 2024-07-09: See if we need this
    ...
    [System.Diagnostics.CodeAnalysis.SuppressMessage(
        ...
        Justification = "Need to use Envrionment to ensure the right newline is generated for platform it is being builtand run on"
    )]
    private static string CleanupSummary(string value)
    {
        ...
        regex = new(
            "<see(?:also)? cref=[\"|'][!,P,T,M,F]+:+OpenSmc\\.Northwind\\.Domain\\.([\\w|\\.]*)(?<generic>`\\d)?[\"|']\\s*/>"
        );
        value = regex.Replace(
            value,
            m =>
                m.Groups["generic"].Success
                    ? $"<code>{m.Groups[1].Value}&lt;T&gt;</code>"
