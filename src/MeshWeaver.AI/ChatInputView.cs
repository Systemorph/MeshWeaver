...
/// <summary>
/// Layout view for the per-user <see cref="ChatInput"/> node. Registers the chat
/// composer as the node's <b>default area</b> (<c>""</c>) so it can be addressed and
/// mounted as
/// <c>new LayoutAreaControl("{user}/_Memex/ChatInput", new LayoutAreaReference(""))</c>
...
/// logic (LoadTemplate / WriteTemplate), which targets <c>{user}/_Memex/ChatInput</c>.</para>
///
...
/// </summary>
public static class ChatInputView
{
    /// <summary>The composer area name — registered as the node's default area.</summary>
    public const string ComposerArea = "Composer";

    /// <summary>
    /// Adds the ChatInput composer view to the hub's layout. <see cref="ComposerArea"/>
    /// is the default area, so an empty <see cref="LayoutAreaReference"/> (<c>""</c>)
    /// resolves to it.
    /// </summary>
    public static MessageHubConfiguration AddChatInputView(this MessageHubConfiguration configuration)
    ...
    /// persists draft + selection to this <see cref="ChatInput"/> node.
    /// </summary>
