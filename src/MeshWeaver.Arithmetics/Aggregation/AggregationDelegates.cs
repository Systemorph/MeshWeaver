...
{
    internal static string GetCacheKey(this IEnumerable<string> identityProperties) =>
        string.Join(",", identityProperties.OrderBy(x => x));
}
    ...
    where T : class
{
    ...
    > AggregateByDelegates =
        new ConcurrentDictionary<string, Func<IEnumerable<T>, IEnumerable<T>>>();
    private static readonly ConcurrentDictionary<
        string,
        Func<IEnumerable<T>, IEnumerable<T>>
    > AggregateOverDelegates =
        new ConcurrentDictionary<string, Func<IEnumerable<T>, IEnumerable<T>>>();
    private static readonly ConcurrentDictionary<
        string,
        (Func<T, T>, IEqualityComparer<T>)
    > GroupByDelegateAndEqualityComparer =
    ...
    )
    {
        ...
        return GroupByDelegateAndEqualityComparer.GetOrAdd(
            key,
            (
                CreateGroupBySelector(properties),
                AggregationFunctionIdentityEqualityComparer<T>.Instance(key, properties)
            )
        );
    }
    ...
    )
    {
        ...
        return AggregateByDelegates.GetOrAdd(
            key,
            CreateAggregationDelegate(GetPropertiesBy(identityProperties))
        );
    }
    ...
    public static Func<IEnumerable<T>, IEnumerable<T>> GetAggregateByDelegate<TKey>(
        Expression<Func<T, TKey>> groupKeySelectorExpression
    )
    {
    ...
    )
    {
        identityProperties ??= new List<string>();
        var key = identityProperties.GetCacheKey();
        return AggregateOverDelegates.GetOrAdd(
            key,
            CreateAggregationDelegate(GetPropertiesOver(identityProperties))
        );
    }
    ...
    private static PropertyInfo[] GetProperties(
        List<string> identityProperties,
        Func<bool, bool> notFunction
    )
    {
        var properties = typeof(T)
            .GetProperties()
            .Where(x =>
                x.HasAttribute<IdentityPropertyAttribute>()
                && !x.HasAttribute<AggregateOverAttribute>()
                && notFunction(identityProperties.Contains(x.Name))
            )
            .ToArray();
        return properties;
    }

    private static Func<IEnumerable<T>, IEnumerable<T>> CreateAggregationDelegate(
    ...
    )
    {
        ...
        var equalityComparer = AggregationFunctionIdentityEqualityComparer<T>.Instance(
            key,
            properties
        );
        return x =>
            x.GroupByKeySelectorAndExecuteLambda(
                    groupBySelector,
                    g => AggregationFunction.GetAggregationFunction<T, T>()(g),
                    identityCopier,
                    equalityComparer
                )
                .ToList();
    ...
    private static Func<IEnumerable<T>, IEnumerable<T>> CreateAggregationDelegate<TKey>(
    ...
    private static Func<T, T> CreateGroupBySelector(PropertyInfo[] properties)
    {
        ...
        if (ctr == null)
            throw new InvalidOperationException(
                $"Type {typeof(T).Name} must have a default constructor"
            );
        var newType = Expression.New(ctr);

        var propertyAssignments = properties.Select(prop =>
            Expression.Bind(prop, Expression.Property(x, prop))
        );
