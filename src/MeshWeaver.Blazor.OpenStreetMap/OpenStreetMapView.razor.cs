...
/// <summary>
/// Blazor view that renders a <c>MapControl</c> with OpenStreetMap tiles via the vendored Leaflet
/// library — no API key and no CDN script; only the tile requests leave the origin. Structurally
/// the twin of the Google provider's view, so a deployment swaps providers purely by which module
/// its <c>Modules:Assemblies</c> lists.
/// </summary>
public partial class OpenStreetMapView : BlazorView<MapControl, OpenStreetMapView>
{
        ...
        if (firstRender)
        {
            try
            {
                jsModule = await JSRuntime.InvokeAsync<IJSObjectReference>(
                    "import", "./_content/MeshWeaver.Blazor.OpenStreetMap/OpenStreetMapView.razor.js");
                await InitializeMap();
            }
            catch (Exception ex)
            {
                Logger.LogError(ex, "Error initializing OpenStreetMap");
            }
        }
        else
        {
            ...
            {
                Logger.LogError(ex, "Error updating OpenStreetMap overlays");
            }
        }
    ...
    private async Task InitializeMap()
    {
        ...
        try
        {
            ...
            await jsModule.InvokeVoidAsync("initializeMap", MapId, mapOptions);
        ...
        {
            Logger.LogError(ex, "Failed to initialize OpenStreetMap");
        }
    }
    ...
    protected override void BindData()
    {
        ...
        DataBind(ViewModel.Id, x => x.MapId, (o, curr) =>
        {
            ...
            return string.IsNullOrWhiteSpace(toString) ? $"osm-map-{Guid.NewGuid().AsString()}" : toString;
        });
