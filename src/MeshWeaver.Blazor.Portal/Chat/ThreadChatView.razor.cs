...
public partial class ThreadChatView : BlazorView<ThreadChatControl, ThreadChatView>
{
    ...
    /// <summary>Header metadata for a delegated sub-thread — what we surface in the
    /// parent's delegation tool-call chip. <see cref="Title"/> is the sub-thread
    /// MeshNode's <c>Name</c> (auto-generated by ThreadNamer or user-edited); <see cref="Icon"/>
    /// is the node's <c>Icon</c> property (defaults to <c>/static/NodeTypeIcons/chat.svg</c>).</summary>
    private record DelegationHeader(string? Title, string? Icon);
