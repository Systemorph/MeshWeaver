...
public class OrleansRoutingService : IRoutingService, IDisposable
{
    ...
    ///
    /// <para>🚨 <b><c>Merge</c>, never <c>StartWith</c> — the ordering IS the correctness (#4013).</b>
    ...
    private IDisposable AttachPodHub(Address address)
    {
        ...
        //
        // 🚨 It is a LEDGER, not a mutex (#4013). It used to be a single token whose "someone is
        ...
        IObservable<bool> ClaimOnce()
        {
            ...
            return Observable
                .Defer(() =>
                {
                    ...
                    // the case that must still claim — see the claimActivity remarks (#4013).
