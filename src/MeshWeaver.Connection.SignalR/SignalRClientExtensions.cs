...
public static class SignalRClientExtensions
{
        ...
        Func<IHubConnectionBuilder, IHubConnectionBuilder>? configuration = null)
    {
        return config
            .WithServices(services =>
            {
                ...
                {
                    return CreateHubConnectionAsync(url, configuration, sp);
                });
    ...
    private static async Task<HubConnection> CreateHubConnectionAsync(string url, Func<IHubConnectionBuilder, IHubConnectionBuilder>? configuration, IServiceProvider sp)
    {
            ...
            async Task Connect(Exception? e)
            {
                try
                {
                    await hubConnection.InvokeAsync(
                        "Connect",
                        hub.Address.ToString(), cancellationToken: CancellationToken.None);
                }
                catch (Exception ex)
                {
    ...
    private static RouteConfiguration AddSignalRRoute(RouteConfiguration routes)
    {
        ...
        return routes.WithHandler(async (delivery, cancellationToken) =>
        {
            ...
            var hubConnection = await routes.Hub.ServiceProvider.GetRequiredService<Task<HubConnection>>();
            logger.LogDebug("Sending message from address {Address}", delivery.Sender);
            await hubConnection.InvokeAsync("DeliverMessage", delivery.Package(), cancellationToken);
