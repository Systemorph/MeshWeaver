    ...
    where TReference : WorkspaceReference
    {
        ...
        if (!owner.Equals(hub.Address))
        {
            ...
            ReArmDecision? ChargeReArmBudget(RecycleRejection rejection)
            {
                ...
                {
                    Interlocked.Exchange(ref sameActivationReAsks, 0);
                    if (Interlocked.Increment(ref recycleReArms) <= MaxRecycleReArms)
                        return new ReArmDecision(rejection, RidingOutSameActivation: false);
                    if (Interlocked.Exchange(ref gaveUp, 1) == 0)
                        logger.LogWarning(
                            "Stream {StreamId}: giving up on owner {Owner} after {ReArms} re-subscribes "
                            + "across DISTINCT owner activations — the address is recycling in a loop, "
                            + "not settling. Readers of this stream will now time out; the fix is at "
                            + "whatever is requesting the recycles, not here.",
                            reduced.StreamId, owner, MaxRecycleReArms);
                    return null;
                }

                if (Interlocked.Increment(ref sameActivationReAsks) <= MaxSameActivationReAsks)
                    return new ReArmDecision(rejection, RidingOutSameActivation: true);
                if (Interlocked.Exchange(ref gaveUp, 1) == 0)
                    logger.LogWarning(
                        "Stream {StreamId}: giving up on owner {Owner} — the SAME activation "
                        + "#{Activation} refused {Attempts} re-subscribes as ShuttingDown without ever "
                        + "releasing the address. That is a WEDGED TEARDOWN at the owner, not a slow "
                        + "reader: readers of this stream will now time out with nothing to show for it.",
                        reduced.StreamId, owner, activation, MaxSameActivationReAsks);
                return null;
            }
