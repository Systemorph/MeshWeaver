			...
			get
			{
				...
				if (isV8)
					if (base.ReadByte(0x7) == 0)
						return Encoding.Default.GetString(Bytes, readoffset + start, len);
					else
						return m_UseEncoding.GetString(Bytes, readoffset + start, Helpers.IsSingleByteEncoding(m_UseEncoding) ? len : len * 2);
				return Encoding.Default.GetString(Bytes, readoffset + start - 1, len);
			}
		}
