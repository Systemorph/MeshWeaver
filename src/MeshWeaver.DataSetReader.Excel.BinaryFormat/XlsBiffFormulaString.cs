	...
	internal class XlsBiffFormulaString : XlsBiffRecord
	{
		...
		public string Value
		{
			get
			{
				...
				{
					return Encoding.Unicode.GetString(Bytes, readoffset + LEADING_BYTES_COUNT, Length * 2);
				}

				return m_UseEncoding.GetString(Bytes, readoffset + LEADING_BYTES_COUNT, Length);
			}
		}
	}
}