...
| Token index | `ApiToken/{hashPrefix}` | The hash and the path of the token row | The token's | Written and deleted under the System identity; the `ApiToken` partition is separately gated and ordinary users hold no grant on it |
...
A supersede is logged at Information as `OAuth: superseding N previous token(s) for user U, client label L: {removed paths} replaced by {kept path}`. The removed path ends in the same hash prefix a superseded holder's `-not-found` line will name, so one search for the prefix ties the removal to the `401` it caused.
