...
| **Reliability & wedges** | [Error Propagation & Wedges](ErrorPropagationAndWedges) — drive wedges to 0 | [Acknowledged But Invisible Writes](AcknowledgedButInvisibleWrites) — a write that reports success and cannot be read back: the two mechanisms, and the one measurement that tells them apart · [Action-Block Wedge Prevention](ActionBlockWedgePrevention) · [Hub Initialization Failure](HubInitializationFailure) · [Orleans Stream Pub-Sub Durability](OrleansStreamPubSubDurability) — a publish with no subscriber succeeds, so a cross-silo reply can vanish with nothing logged · [Durable Streams Are Mesh Nodes](DurableStreamsViaMeshNodes) — the design that retires the memory stream without a provider |
