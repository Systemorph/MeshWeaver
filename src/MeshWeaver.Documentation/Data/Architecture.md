...
- [Source Nodes Have No Link Gate](SourceNodesHaveNoLinkGate) — the link gate measures a module's BYTES; an in-mesh Code node is text, so nothing measures it until Roslyn does. Six NodeTypes on memex-cloud stopped compiling forty minutes after a satellite merged source calling two core APIs the running image predates, and the `CS1061` on a platform type is what tells that apart from a content bug
...
- [Source Nodes Have No Link Gate](SourceNodesHaveNoLinkGate) — the gate that covers a module's bytes does not cover the Code nodes a package delivers beside them; the measured window between a satellite merge and the portal that cannot compile it, and the three commands that tell platform skew from a missing source node
