...
- [The Initialization Budget Ladder](InitializationBudgetLadder) — initialization is a nested wait and every level took the same 120 s constant; equal is not an ordering, so when the enclosing bound won the level that knew which wait starved was torn down as a recognised shutdown and recorded nothing. A hub born inside another hub's initialization now takes a strictly contracting rung — and HOSTED is not ENCLOSED, so a per-node hub keeps the full 120 s for its whole initialization while its own time-box, one rung in, reads 115 s
...
- [The Routing Entry Point](RoutingEntryPoint) — every routed message pays an Orleans grain PLACEMENT; the two measured ways it fails and drops live traffic, and the case for a local routing service where a silo is co-hosted
...
- [Closed Type Set](ClosedTypeSet) — `Mesh:ClosedTypeSet=true`: a process activates only the NodeTypes its image registers in code; a database-defined type is refused at activation, never enumerated by the sweep, never adopted and never compiled — the property the dedicated control instance's readiness rests on
...
- [Static Repo Import](StaticRepoImport)
- [Module Sync Per Manifest Hash](ModuleSyncPerManifestHash) — an instance always syncs every module it has: each module is judged alone by the content hash in its manifest.lock (unchanged writes nothing, changed syncs, a declared platform floor above the running platform declines that one module), and the seal decides only adoption versus compile — never whether sources arrive
...
- [The Platform Compatibility Ladder](PlatformCompatibilityLadder) — platform builds are backwards compatible within a major and a compatibility epoch, so plugin bytes built on N run unchanged on N+1; the floor/ceiling/epoch that bound it, the machine-checked break declaration, the API-break catalogue (static verdict vs what really happens at run time), the check that blocks every platform pull request on the deployed plugin set, and what no surface check can see
...
- [Every HttpClient Names Its Pipeline](EveryHttpClientNamesItsPipeline) — `Source: '-standard//…'` was the shared defaults pipeline every non-re-registered client rode under one empty instance and one circuit breaker, not an unnamed client; the pipeline instance is now the client's own name, and the timeouts behind it were the self-update hand-over meeting a restarting control instance
...
- [The Compile-State Satellite Is Retired](CompileStateSatelliteRetired) — a phase-1 dual-write nobody ever read cost one node-operation round trip per NodeType activation and one grain activation per NodeType per state move; measured as the destination of boot-time placement timeouts, stalled activations and routing back-pressure legs after the compile lane moved off the ThreadPool, and removed rather than batched
- [Compiling Off the ThreadPool](CompileOffTheThreadPool) — a NodeType compile held the silo's shared ThreadPool twice over (its async leaf ran Roslyn on a pool worker, and ConcurrentBuild fanned each emit out onto the pool); the probe that measured it, the fix, and the roll-time routing starvation it bears on
- [Blocking Leaves Off the ThreadPool](BlockingLeavesOffTheThreadPool) — every IIoPool's blocking leaves borrowed ThreadPool workers under caps up to 256, and a kernel cell's compile ran on one; what the production stall readings (#5388) say was GC and what was blocking, and the fix
...
- [Collectible Thread-Static Handle Reuse (CoreCLR)](CollectibleThreadStaticHandleReuse) — a thread exit frees a slot in a LIVE collectible context's handle table when a thread-static index was reused after an unload; sighting #18's dangling static base, reproduced deterministically
...
- [The Canonical Node-Test Harness](CanonicalNodeTestHarness) — `run-node-tests.py` has one home and the satellites launch it; how three vendored copies broke silently and differently the day the compile model changed, and why the one-line revert is measurably not the fix
...
- [Self-Update Announcement Key](SelfUpdateAnnouncementKey) — a deployment that must not hold the fleet inbox secret announces its own self-update with a key of its own; the key causes nothing but that record's self-update events, and a record that declares one is no longer announceable with the fleet secret
...
- [Deploying Across Platform Versions](DeployingAcrossPlatformVersions) — the ladder: platform and plugins roll separately; a seal gates a roll only behind a declared break
...
- [A Stale Run Listing Is Not a Broken Main](StaleRunListingRefusals) — GitHub serves the platform resolver a weeks-old page of workflow runs, per call and with no error (three measured); why only one of the two listings can be checked, why neither retries, and the refusal wording that sent two readers at a `main` that was fine
...
- [Closing Keywords and Issue State](ClosingKeywordsAndIssueState) — GitHub binds a closing keyword to the one reference after it and reads nothing else, so "Does not close #N" closes #N; the three misfires of one day (a disclaimer, a partial claim, and a docs PR narrating the first two), why a severity label may not be closed by a merge at all, the three-check gate, the per-issue escape that keeps it from being a wall, and the four blind spots it keeps
...
- [The CD Ledger Records a Failure, Not a Cadence](CdLedgerRecordsFailures) — 109 of 109 `incomplete image set` alarms recorded a delivery that had not failed: the pair tag that cannot be satisfied because Plugins merges faster than a publish takes, the minutes before a publisher even exists, the registry fingerprint that separates an attempt from an absence — and the rebuild-cadence decision this deliberately leaves open
