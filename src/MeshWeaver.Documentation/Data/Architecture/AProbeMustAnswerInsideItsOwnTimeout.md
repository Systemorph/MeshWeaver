...
`startupProbe` with `timeoutSeconds: 10`, `periodSeconds: 10` and `failureThreshold: 1080` (3 h on
that instance, raised deliberately to fit a cold bake).
...
- Checks at or above 10 ms are **named**, slowest first. Ten milliseconds is a hundredth of the
  smallest probe budget the fleet runs, so nothing below it can be part of an explanation for a
...
raise `timeoutSeconds`: a bigger budget would move the cliff without removing it, and the next
...
- the request timeout is 20 s against a 10 s probe budget, so the probe never waits long enough to
  see it fire;
