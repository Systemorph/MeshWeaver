...
| A retry / re-probe that re-reads the ambient `AsyncLocal` from inside a NACK's `OnError` callback or a timer | `hub.Observe` restores the caller's identity on `OnNext` only; an `OnError` callback runs under whatever the NACK delivery carries, and the Orleans router's `ShuttingDown` NACK for a deactivating grain (`RoutingGrain.PostFailure`) carries **none**. `GetMeshNode`'s recycling re-probe did exactly this and was failed closed on `portal/reads-{meshId}` — the `hub=portal/reads-…, message=GetDataRequest, target=<page path>` family (#5227, #5231, #5232, #5431, #5433–#5435, #5438, #5439). | Capture the identity ONCE where the operation starts and pin it on every attempt with `o.WithAccessContext(caller)` — the re-probe is the same read, so it carries the same identity. Pinned by `GetMeshNodeReProbeKeepsCallerIdentityTest`. |
| A Blazor component issuing a read or write through the mesh-scoped `IMessageHub` / `IMeshService` from a lifecycle method (`OnParametersSet[Async]`, `OnInitialized`) | In a circuit scope both resolve to the ROOT MESH HUB, so a read goes out from `portal/reads-{meshId}` (a hub with no user) carrying only the AMBIENT identity. `CircuitAccessHandler` sets that `AsyncLocal` for an inbound activity and nothing else — and Blazor runs a lifecycle method in whatever render re-sets the component's parameters, including one a parent dispatched with `InvokeAsync(StateHasChanged)` from an Rx callback, which the renderer executes on the CALLER's execution context (`RendererSynchronizationContext` runs it inline when quiescent, else in a continuation of the caller's context). There the identity is null and the post is refused: `ApplicationPage`'s pre-rendered-body read produced the `hub=portal/reads-…, message=GetDataRequest, target=<page path>` line that kept recurring after the re-probe fix above (#5227, #5573). | Issue the call on the circuit's portal hub (`PortalApplication.Hub`, which `BlazorView` exposes as `Hub`): its post pipeline stamps the circuit user from `ICircuitContextAccessor.UserContext` on every post, whatever thread it runs on. Pinned in MeshWeaver.Plugins by `ApplicationPagePrerenderIdentityTest`. |
| The same read, one hop removed: a Blazor component HANDING the mesh-scoped hub to a helper that reads through it, or reading from a handler Blazor runs off an inbound activity (`OnNavigateAsync` for a `NavigateTo` issued from an Rx callback) | The line kept recurring after `ApplicationPage` was fixed (47 failed reads in six hours on the image that carried it, all `hub=portal/reads-…`, targets `Store`, `Home`, `AgenticEngineering`, `SST`, `Pricing`, `LossModelling`). Three more readers of the same shape were found: `UcrLink` (the name lookup behind every `@/…` link, `OnParametersSet`), `CreateNode`'s pre-selection, and `Routes`' public-page decision, which PASSED the root hub to `SeoResolver` from `OnNavigateAsync` — and the anonymous gate's `RedirectOnDenied` navigates from an Rx callback, so that handler ran with no identity. 🚨 A repro must WARM the path-resolution cache first: a cold `IPathResolver` answers from its query's system-scoped continuation, so the read then rides `system-security` and the old code passes. | Issue the read on `PortalApplication.Hub`, or — where the helper is built for the root hub, as `SeoResolver` is — run it inside the instance extension `accessService.RunAs(() => circuitContextAccessor.UserContext, () => work)` (the `Func<AccessContext?>` overload, which resolves the identity at subscribe) so the helper captures the circuit's identity at subscribe. Held structurally in MeshWeaver.Plugins by `ComponentReadsIssueOnThePortalHubGuard` (no component issues `GetMeshNode`/`GetMeshNodeOutcome`/`GetPreRenderedHtml` on the injected root hub; hand-offs of it are a shrink-only reviewed list), and pinned by `UcrLinkNameReadIdentityTest` and `RoutesReconnectStateTest.AnonymousNavigationStartedOffTheInboundActivity_StillLeavesTheCircuit`, each of which logs the production line verbatim on the old code. |
| An ASP.NET middleware registered BEFORE `UserContextMiddleware` that reads the mesh through the root hub | No `AccessContext` exists for the request yet, so a helper that captures the ambient identity (`SeoResolver.Resolve`) captures null and its read leaves `portal/reads-{meshId}` with none. `PublicSite.UsePublicHostRedirect` did exactly this for its "is this page public?" decision. Its fail-open turned every refusal into "not public", so the app host served public pages with `200` instead of redirecting them. This was the steady-state producer of the `hub=portal/reads-…, message=GetDataRequest, target=<space root>` line on memex.meshweaver.cloud: one refusal per synthetic-probe run, within a second of the probe's `GET /Store` on the app host (#5227). | Resolve the caller explicitly with `UserContextMiddleware.ResolveHttpCaller(http.User, services)`, the resolver for surfaces that run outside the middleware (Anonymous for an unauthenticated request), and run the read inside `accessService.RunAs(caller, () => work)`. Pinned by `PublicHostRedirectAsksAsTheStrangerTest`, which runs the DEFAULT decision over a real mesh; `PublicSiteTest` stubs the decision and could not see it. |
