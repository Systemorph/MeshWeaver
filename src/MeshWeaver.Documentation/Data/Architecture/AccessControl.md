...
**And read it ONCE per create.** The three validators above run back to back on one validation context and share a single resolution (`PartitionOwningTypes.OwnsPartitionOnce`); the post-creation handler, on the far side of the storage write, deliberately keeps its own. Why that split, and why a NESTED instance of an in-mesh owning type is still not refused, is [Partition Ownership Resolution](/Doc/Architecture/PartitionOwnershipResolution).

**Still open (#4449):** a NESTED instance of an in-mesh owning type is not refused — the static-type refusal never covered in-mesh types, and [Partition Ownership Resolution](/Doc/Architecture/PartitionOwnershipResolution) measures what closing it would cost. (The two items beside it are done: a bulk `CreateNodesRequest` now compensates a failed owner grant — [Bulk Create Compensation](/Doc/Architecture/BulkCreateCompensation) — and the declaration is resolved once per create rather than once per check.) `RlsNodeValidator` is registered by `AddRowLevelSecurity()` alongside `PartitionWriteGuardValidator`, `OwnsPartitionProvisioningValidator` and `PartitionRootDeletionGuard` — validators **AND**-compose, so a rejection by any one of them wins even when RLS would grant.
