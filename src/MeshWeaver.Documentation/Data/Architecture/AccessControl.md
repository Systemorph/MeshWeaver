...
What it grants is what any signed-in user already had — creating a partition, with themselves as its Admin. `top-level ⟺ owns a partition` still holds: a non-owning type is refused at the root, and the logged-out caller (who arrives NAMED `Anonymous`) is refused by `WellKnownUsers.IsAuthenticated`. Space and User keep their own rules and handlers; the in-mesh handler matches STRUCTURALLY (a partition root whose type is not registered in `src/`), the creation-side twin of the [partition teardown](/Doc/Architecture/PartitionTeardown). Pinned by `InMeshPartitionOwnerTopLevelCreateTest`. `RlsNodeValidator` is registered by `AddRowLevelSecurity()` alongside `PartitionWriteGuardValidator`, `OwnsPartitionProvisioningValidator` and `PartitionRootDeletionGuard` — validators **AND**-compose, so a rejection by any one of them wins even when RLS would grant.
