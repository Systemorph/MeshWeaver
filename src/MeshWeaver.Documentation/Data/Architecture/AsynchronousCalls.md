...
- Don't paper over it with default-interface Task shims, and don't keep a Task overload "for tests": 🚨 **`.ToTask()` is forbidden in tests too** (maintainer, 2026-08-30) — and **`await someObservable` is not the lighter alternative**, it is the same inline resume with nothing to grep for. A test asserts on the stream (`await x.Should().Within(TestTimeouts.Convergence).Emit("because")`); where an `async Task` signature genuinely must take a value out of one, it waits through `.Await(ct)` or `.ObserveCompletion(reportLateFault, ct)`. The production interface stays Task-free.
...
🚨 **There is no sanctioned `.ToTask()` boundary — not even at the test edge** (maintainer, 2026-08-30: *"no ToTask ever"*). The mechanism below is the reason: a Task completed inside an Rx pipeline resumes its awaiter INLINE on the signalling thread, still inside the trampoline, and everything the continuation does inherits that flag — which is why this was never a test-only concern. 🚨 **And awaiting the observable DIRECTLY is the same defect, not the fix** — Rx's awaiter is an `AsyncSubject<T>` that completes its continuation from inside `OnCompleted`, so `await …FirstAsync().Timeout(…)` and `await …Take(1).Timeout(…)` resume on the signalling thread exactly as the bridge does ([Removing Observable→Task Bridges](../RemovingObservableToTaskBridges) measures it). This paragraph prescribed that shape until 2026-09-18, which is where the 228-site `DirectObservableAwaitSites.allow` inventory came from. Stay reactive and `.Subscribe`; where a foreign signature forces a `Task`, wait through `.Await(ct)` or `.ObserveCompletion(reportLateFault, ct)`; in framework lifecycle hooks (`OnActivateAsync` etc.) subscribe and return `Task.CompletedTask`. The one place a bridge may work is inside an ACTIVITY, where nothing mesh-side runs after the await — and even there, prefer the reactive shape.
...
Obtain a pool with `hub.ServiceProvider.GetService<IoPoolRegistry>()?.Get(IoPoolNames.X) ?? IoPool.Unbounded` (the `Unbounded` fallback still offloads to the ThreadPool with `ConfigureAwait(false)`, so it is never worse than the bare `FromAsync` it replaces). Pool names live on `IoPoolNames` — `FileSystem`, `Blob`, `Http`, `Ai`, `AgentStore`, `Query`, `Layout`, `Routing`, `Compile`, `Process`, plus the per-provider prefixes `pg:`/`pg-read:`/`sf:`/`sf-read:` (`pg:{provider}` is capped at 1 for writes and `pg-read:{provider}` at 16 for reads — together a budget under the shared data source, not a mirror of one connection) — and pick the concurrency cap per resource class. Read the XML docs on `IoPoolNames` before picking one: several exist to make nesting acyclic (`AgentStore` is deliberately NOT `Ai`, because a store call runs inside a tool call that already holds an `Ai` slot) or to make a subscribe drainable at teardown (`Query`, `Layout`) rather than to throttle. For idempotent one-shots, cache the eager `pool.Run(...)` observable in an instance `ConcurrentDictionary` (the **promise-cache** — canonical: `PostgreSqlPartitionStorageProvider.EnsurePartitionProvisioned`).
...
| **I/O leaf with a driver pool** | Postgres / Cosmos adapters & query leaves | **Per-provider `IIoPool`** — `pg:{provider}`, cap 1 for writes plus `pg-read:{provider}`, cap 16 — together a *budget* under the shared `NpgsqlDataSource`'s `MaxPoolSize=50`, **not** a mirror of one dedicated connection (see [Controlled I/O Pooling](/Doc/Architecture/ControlledIoPooling)). The hot query/storage `FromAsync` sites are **migration debt, not a sanctioned pattern**; new code (e.g. `EnsurePartitionProvisioned`) is pooled from day one. |
