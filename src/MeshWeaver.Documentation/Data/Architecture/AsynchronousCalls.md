...
**Second canonical reference: delegation sub-thread spawn.** `ChatClientAgentFactory.ExecuteDelegationAsync` subscribes to `meshService.CreateNode(subThreadNode)`. The Subscribe callback would otherwise fire on the mesh-service reply thread — racing the parent thread hub's ActionBlock when calling `AgentChatClient.EmitDelegationEvent` (which mutates `_activeDelegationPaths` AND emits onto the `Delegations` Subject that hub-pipeline code reads). Fix: post `DelegationDispatchedTrigger` to the parent thread hub's address; the registered `HandleDelegationDispatched` handler emits the event AND sets up the sub-thread node subscription, all serialised by the ActionBlock. The sub-thread node Subscribe inside the handler in turn posts `DelegationTerminalTrigger` on Running→Idle transition so the Terminal event also fires under the ActionBlock.

**Type-registry note:** any `Post` of an internal trigger must be registered on **every hub the message transits**, including the originator. `AIExtensions.AddAITypes` registers `DelegationDispatchedTrigger` + `DelegationTerminalTrigger` for that reason — without the type on the originating client hub the JSON discriminator falls back to FQN and the receiving hub's short-name lookup misses (see `DebuggingMessageFlow.md` → "Type-registry mismatch").
