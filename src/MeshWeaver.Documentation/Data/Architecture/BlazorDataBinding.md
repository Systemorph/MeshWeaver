...
For the full explanation of why `await` deadlocks hub-touching operations, see [Asynchronous Calls](/Doc/Architecture/AsynchronousCalls).
...
- [Asynchronous Calls](/Doc/Architecture/AsynchronousCalls) — the full explanation of why `await` deadlocks hub-touching operations.
- [CQRS — Queries vs. Content Access](/Doc/Architecture/CqrsAndContentAccess) — choosing the right read primitive for each situation.
- [Workspace References](/Doc/Architecture/WorkspaceReferences) — what each `WorkspaceReference<T>` shape emits and when to use each.
