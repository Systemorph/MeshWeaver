...
`IMeshNodePersistence` is registered as a **scoped service** — each hub gets its own instance with the correct `IMessageHub` injected, following the same pattern as `IPersistenceService`. The identity (`createdBy`, `updatedBy`, `deletedBy`) is automatically resolved from `AccessService.Context.ObjectId`, so callers don't need to pass it explicitly.
...
// All operations now use hub.Address as identity and AccessContext
