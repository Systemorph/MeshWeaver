...
- **The trace log's `alc=1` against three resident contexts.** `alc=` counts `AssemblyLoadContext.All`
  (`MonolithMeshTestBase`), a registry a context leaves on `Unload()` — yet all three read `_state = 0`,
  so `Unload()` was never called on them. Whether the three were constructed after the last checkpoint
  that carried `alc=`, or `All` had dropped them for another reason, was not measured; do not cite
  `alc=1` as "no collectible context" again.
...
- **A collectible-ALC UNLOAD overlap — but not collectible ALCs.** The first reading of this dump said
  *absent: one ALC, 133 modules, none duplicated*; fact 6 re-measured it as **three** resident
  ...
  above). What **is** eliminated is the #12–#16 mechanism: no context is mid-unload, zero
  `DISPOSE_ALC_RETAINED`, and the death is inside the current instance's own dispose with no next
  instance started. The 2026-09-11 diagnosis and the fixes built on it (core #4042/#4046/#4048/#4053,
