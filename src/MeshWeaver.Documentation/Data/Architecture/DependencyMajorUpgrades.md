...
`HTTP=404 BYTES=14` that a hurried reader skims as evidence. That is this repository's own
"a verification step that cannot fail is not a verification step" rule, violated in the one place it
is being taught. Make the command *refuse*:
...
the `-s` test refuses a zero-byte success. Measured while writing this page: a guessed path for Rx's
suppression file answered **HTTP 404, 14 bytes**, and a `grep -c` over that file scores **0** — which,
unguarded, reads as "zero suppressions, all clear".
...
dependent's next pin move, long after this repository went green. This page's own YamlDotNet entry is
an instance: 18.0.0 added `HasParseMethod` and `Parse` to `ITypeInspector` with no default
...
types, and new overloads are caller-only and safe to skim. Additions of abstract or
default-less members to a type an outside assembly may implement are **removals in disguise**, and
get the same treatment: enumerate them, then search both repositories for implementers. (Core
carries the mirror-image obligation on its own surface — the
`Interface additions (implementers declared)` gate — for exactly this reason.) One more caller-side
trap that an addition can cause here: a new overload can make a dependent's `<see cref>` ambiguous,
`CS0419` under `-warnaserror`.
...
| API additions (the implementer half of step 3) | **Nothing implementer-obliging.** All 16 additions are methods on ONE type, `System.Reactive.Linq.ObservableEx`, which is static (69 methods, no constructor) — so they are caller-only overloads. **Zero** public types were added, and no member was added to any interface or abstract class, so there is no `CS0535` shape for a satellite to meet at its next pin move. |
...
| Suites | **15 suites, 6,944 tests, 0 errors, 0 failures** (2 skipped), run as the test executables against the merged tree: Graph 1545 · Memex.Portal.Shared 1293 · Compiler.Pipeline 859 · Hosting 619 · Data 508 · Layout 496 · Messaging.Hub 409 · Documentation 397 · PluginTester 362 · Hosting.Orleans 259 · ContainerImages 112 · ContentCollections 28 · Testing.Xunit 23 · Cli 19 · Deployment.Contract 15. (`MeshWeaver.Security.MeshTest` is not a suite — `OutputType=Library`, zero `[Fact]`/`[Theory]`.) |
