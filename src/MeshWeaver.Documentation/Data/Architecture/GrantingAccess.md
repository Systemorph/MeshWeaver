...
> 🚨 **A SYSTEM-OWNED partition grants nobody write — and a grant COMMITTED TO A REPO can never be one.** A partition with a **one-way** `{partition}/_GitSync` is rewritten from its repo on every sync, so the only identity that may write it is `system-security`. `AccessAssignmentGuard.IsForbiddenOnSystemOwned` refuses any `Admin`/`Editor`/unknown-role grant there (`Viewer`/`Commenter` entitlements and every `Denied` assignment stay legal), and `SystemOwnedAccessRetractionHandler` deletes such grants when the `_GitSync` is wired — so the shape is unsatisfiable, not merely inconvenient. A **bijective** (`twoWay: true`) sync is a different contract and is **not** system-owned: there the mesh nodes are the working copy and the people editing them keep write access.
>
> 🚨 **Two rules used to meet head-on here, and the end state above was unreachable as a result** (#5140). The retraction sweep deliberately SPARES a partition's last administrator rather than issuing a delete the pipeline would refuse — and `SpaceAdminInvariantValidator` ("a space must always have at least one admin") refused exactly that delete, because it exempted only system-managed mirror partitions. So a repo-owned space kept one human `Admin` grant for ever: the sweep spared it on every run, the guard refused any second Admin that would have let it go, and the grant still conferred write over content whose edits the next sync reverts. The invariant now exempts a system-owned partition too — its administrator is the importer identity, not a person — so the sweep converges. Reading a `_GitSync` partition that still has a human `Admin`, check the log for *"Allowed last-admin removal on SYSTEM-OWNED partition"* (it ran) before assuming the sweep did not.
>
