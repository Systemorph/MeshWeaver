...
### A forced teardown ENDS its readers — a held read is never left a silent corpse (#5011)

Two teardowns ignore the subscriber count: `Invalidate` (run after a node delete commits) and the cache's own `Dispose`. Both dispose the entry's hydration and its upstream sync stream — and with the upstream goes the stream's heartbeat. Until #5011 neither told the readers still subscribed to the entry: its `Replay(1)` subject was never written again, never completed and never faulted. A reader holding it received no value, no error and no completion, forever, while the thing it held had stopped keeping anything alive.

That is the worst shape for a **process-lifetime holder**. The Hosting package's inbox anchor holds `GetMeshNodeStream` of the fleet watch's owner (`Hosting/PlatformBuilds`) open in every portal process, precisely so the held stream's heartbeat keeps that owner activated. A holder whose entry was torn down this way reads as "holding" while it holds nothing, and it has no terminal on which to log the loss.
