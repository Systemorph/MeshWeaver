...
The caller's terminal is untouched — `req.Result` is fed exactly as before, so nothing on a GUI waits longer than it used to. Only the next **queued** write waits, and only for the one fact it cannot proceed correctly without.
