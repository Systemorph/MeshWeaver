...
> **Frame version = `Hub.Version` floored at a per-activation content baseline.** The sync-stream *frame* version (what the receive-side monotonicity guard compares) rides `Hub.Version` — reliably monotonic **within** an activation (the owner `++`s it per message) — but `Hub.Version` **resets to 0 on every (re)activation**, so on a multi-replica portal a mirror on another silo that cached the higher pre-recycle frame version DROPS the recycled owner's low post-recycle resubscribe Full under the guard (`version < Current.Version`) and stays orphaned — the "index vs node-resolution split-brain" residual of #325 (symptom 2), observable only cross-silo (the monolith heals it via the heartbeat resubscribe).
>
> The fix is the **frame-layer analog of `NextVersion`**: a MeshNode data-source stream floors every frame it originates at a **per-activation content baseline** — the highest persisted `node.Version` present in the store, **loaded once at activation and frozen**. Concretely `SynchronizationStream.StampVersion` (used by `ApplyChanges`) and `OwnerVersion()` return `max(Hub.Version, baseline)`; the baseline is supplied by `MeshDataSource.WithOwnerVersionBaseline`. Because the persisted `node.Version` is minted forward-only (`NextVersion`) and the owning per-node hub processes at least as many messages as the data-source sync hub, the frozen baseline **dominates every frame version the mirror saw before the recycle**, so the post-recycle resubscribe Full is *accepted* and the mirror converges.
>
> Crucially the baseline is a **stable per-activation value, never the per-frame content version.** A partial cross-hub patch carries no `Version` field (its reduced content reads `0`); flooring at *that* made the frame version flap `7 → 0 → 11` and dropped legitimate mid-stream frames — the reason the naive floor was reverted (it broke the activity/export relay). Capturing the baseline once and freezing it avoids the flap. **Layout-area / ephemeral / non-MeshNode streams supply no baseline, so their frame version stays `Hub.Version` verbatim** — the layout-Full render path is untouched (the 2026-06-18 "cannot find pinned doc" wedge cannot recur). Proven by the deterministic 2-silo repro `TwoSiloRecycleConvergenceTest` (orphaned-on-main → converges-with-floor) and guarded by `ExportDocumentScriptRelayTest` / `DataChangeStreamUpdateTest` / `InlineEditingTest`.
