...
| Either version **unknown** | The head moves, as before. An absent version is no evidence of order (R2). |
| A newer head whose **entry DLL is gone** | The head moves. A record without bytes is not a version this registry holds, and refusing the one upload that could heal it would turn the rule into a self-sealing outage. |
