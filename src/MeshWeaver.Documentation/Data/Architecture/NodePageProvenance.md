...
Description: The Type · Created · Updated line rode on one renderer, so a node type that drew its own landing page dropped it silently — 86 of 99 pages across the fleet had. What the census found, why the default was inverted rather than the call sites patched, how a page declines the line in a sentence, and the two halves of the problem core cannot reach.
