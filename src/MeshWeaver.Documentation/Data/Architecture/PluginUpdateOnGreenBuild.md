...
## 🚨 Reminder by default; unattended only when asked

A changed module raises a `Notification` satellite on the install record — the bell surfaces it and
the catalog card offers **Update**. Installing happens **without a human only** for a package whose
install record opts in.

That default is deliberate: a bad plugin build reaching every installation unattended is a much worse
failure than an update that lands an hour late. Opt in per package, for the ones you trust.
...
### 3. Opt a package in to unattended updates

Off by default. Turn it on per installed package, once you trust that plugin's CI.
...
- **A module changed** → an "Update available" notification on the install record, naming how many
  files changed and removed, and the short sha it was built from.
- **A module changed and is opted in** → the delta install runs, touching only the changed files.
