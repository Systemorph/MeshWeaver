...
4. **Lock, then open a fresh attempt** — upsert `{P}/_Activity/import-{fingerprint}` (the node lifecycle makes the **first caller win**; concurrent replicas get "already exists" and stop). That deterministic node is the **lock** and the durable "version vN imported at T" record — nothing else. The run's own log goes to a **fresh** `{P}/_Activity/import-{fingerprint}-{timestamp}-{rand}` node, one per attempt. See *Content-addressed Activity — the lock + the short-circuit* below for why the two roles are split.
...
**The lock** — `{Partition}/_Activity/import-{fingerprint}`, id = the fingerprint:
...
- **Changed source ⇒ new id** ⇒ a fresh import runs. Old `import-{prevHash}` locks remain as a visible import history.
- It is written **only** through the idempotent upsert (`CreateOrUpdateNodeRequest`), twice per run: `Running` on acquisition, then the terminal verdict. That verb floors the version on the durable row it just read, so a forked/ghost lock row is repaired in place rather than silently discarded (#902/#909).
