...
| site | growth | status |
|---|---|---|
| `JsonSynchronizationStream.CreateExternalClient` registered the owner-protocol subscription on the SUBSCRIBING hub as well as on the stream | **+1 per remote stream ever opened**, holding the whole `SynchronizationStream` graph (measured: floor 4 → 6 over two streams, still 6 after both were disposed) | **Fixed.** The duplicate bought nothing: the stream's own composite rides its `sync/` sub-hub, which is a hosted hub of the subscribing hub and is therefore disposed in the `DisposeHostedHubs` phase — strictly BEFORE that hub walks its own registrants in `ShutDown`. Pinned by `StreamRegistrantsLeaveTheSubscribingHubTest` |
