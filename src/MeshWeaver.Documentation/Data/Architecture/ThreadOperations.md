...
### 🚨 `StartThread` wakes the thread hub — creation alone never started a round

A per-node hub is instantiated by the FIRST message routed to its address, and the `CreateNodeRequest` that makes a thread is handled by the OWNER namespace's hub — it anchors the node and writes the store, and the thread's own hub, whose init installs the submission watcher that drains the seeded message, is never touched. Every caller that subscribed to the new thread's stream afterwards (the chat view navigating to it, a delegation awaiting its sub-thread, an MCP caller polling `get`) activated the hub by accident; a caller that only recorded the path left its thread at version 1 with the task `Submitted` forever. Measured on the control instance on 2026-09-20: 100+ `Hosting/Triage/_Thread/*` threads, one per red CI run since 2026-09-19, and the first `get` on one of them ran its round within a minute.

`StartThread` therefore owns the activation (`HubThreadExtensions.WakeThreadHub`): after the owner confirms the create it subscribes ONCE to the new thread's own stream through the mesh-wide `IMeshNodeStreamCache`, off the router, and releases on the first emission — the freshly activated hub answering its own node. 🚨 The caller's context, captured on the caller's frame, is supplied as the **subscriber context** — the identity the cache's per-subscriber `Read` check evaluates — and nothing more: the shared upstream keeps `MeshNodeCacheIdentity`, and a wake must never stamp a user onto it (`MeshNodeStreamCache` builds its hub `WithPostingIdentity(PostingIdentity.System)`). What the capture buys is that the subscriber check has a REAL identity instead of null: a null context fails the read closed, which leaves the thread exactly as parked as before. A service inside `ImpersonateAsSystem` therefore wakes as system, a circuit as its signed-in user, and neither as nobody. Idempotent with a viewer's own subscription (the cache shares one upstream per path). Pinned by `ThreadHubWakeOnStartTest`, which observes the STORE through the query index — the one read that cannot itself wake a hub — and whose control is the same node written by the create verb alone: parked for five seconds of polling, then run on its first stream read. A creator that dies between the create and the wake is what the [thread supervisor](/Doc/Architecture/ThreadSupervision) covers.
