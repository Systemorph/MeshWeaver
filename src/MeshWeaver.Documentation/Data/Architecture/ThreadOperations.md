...
`StartThread` therefore owns the activation (`HubThreadExtensions.WakeThreadHub`): after the owner confirms the create it subscribes ONCE to the new thread's own stream through the mesh-wide `IMeshNodeStreamCache`, off the router, under the caller's identity captured on the caller's frame (a service inside `ImpersonateAsSystem` wakes as system; a circuit wakes as its user; neither null, which would fail the read closed), and releases on the first emission — the freshly activated hub answering its own node. Idempotent with a viewer's own subscription (the cache shares one upstream per path). Pinned by `ThreadHubWakeOnStartTest`, which observes the STORE through the query index — the one read that cannot itself wake a hub — and whose control is the same node written by the create verb alone: parked for five seconds of polling, then run on its first stream read. A creator that dies between the create and the wake is what the [thread supervisor](/Doc/Architecture/ThreadSupervision) covers.
