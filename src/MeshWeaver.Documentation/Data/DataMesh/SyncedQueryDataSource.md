...
- When an explicit `IMeshChangeFeed` event reports a path change that no mirror can be held to — a delete, a recreate, or the version-less recycle broadcast — open subscribers stay attached to their existing stream while new callers bind to a fresh one tied to the re-activated owner. A versioned `Updated` (one per write) does **not** evict: the workspace keeps the stream and a writer's base must reach the announced version instead ([#1174](https://github.com/Systemorph/MeshWeaver/issues/1174), see [Live Mirrors and the Change Feed](/Doc/Architecture/LiveMirrorsAndTheChangeFeed)).
