...
Give a pure grouping parent the **`NodeMenuItemDefinition.GroupArea`** sentinel (`"_group"`) — the sibling of the long-standing `"_separator"`. It makes the wire self-describing: a client that cannot nest can still tell "this is a group, not an action" instead of rendering a dead row that navigates to `/{path}/`.

### The aggregator normalizes the tree

Two things happen once, in `RenderMenus`, so every renderer can stay dumb:
