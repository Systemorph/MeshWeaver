...
| **Owner Injection** | The node OWNER (from `CreatedBy`) is the standing access context on a node/thread/activity hub — injected everywhere, carried forward via that hub's standing identity (`AccessService.SetStandingIdentity`), never a process-wide field. Genuine infra (doc sync) = System; an empty context is rejected, never faked. See [Owner Injection](/Doc/Architecture/OwnerInjection). |
