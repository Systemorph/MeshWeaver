...
- **`PermissionEvaluator`** is hub-scoped, fully local, observable; `GetPolicy` / `GetPermissionRequest` flow through observable streams with a synchronous fast path for built-in roles and a per-user scope-roles cache.
...
- **Replace `Task`-returning surfaces with `IObservable<T>`** in any user code that talked to `IMeshService` / `IMeshStorage` / `PermissionEvaluator` / navigation. The old async extensions are `[Obsolete]`. Subscribe to fire side effects.
