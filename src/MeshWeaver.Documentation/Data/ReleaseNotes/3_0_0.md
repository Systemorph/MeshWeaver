...
current sanitiser and a guard test keeps it there. Every other warning has a measured disposition
on that page: two are false positives (the icon route synthesises an icon for any name; `TRACE`
and `OPTIONS` answer 405), two more leave with the old editor loader, and the content-security
policy and the cross-origin headers are carried by design, with the reasons written beside them.

- [The full 3.0.0 scan report](/Doc/Architecture/SecurityScan_3_0_0) — attack classes exercised, the delta against the 23 August scan, live header verification, CORS posture, limitations, and the raw verdict output.
