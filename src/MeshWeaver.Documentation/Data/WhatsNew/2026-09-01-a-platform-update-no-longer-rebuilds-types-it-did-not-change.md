...
Category: Fix
Description: When a platform update leaves a type's code untouched, the portal now checks that instead of assuming the worst — and it catches the opposite case too, where the code did change and nothing noticed.
Icon: Sparkle
Order: -20260901
---
...
too broad — the set of things counted as "the tools" moves on most changes to the platform, so a
release that touched none of your code still declared every compiled type suspect and rebuilt them
all.
...
After an update, when a type is about to be rebuilt but a usable build is already available, the
portal reconstructs what a fresh compile would be handed and compares it with what the existing
build was made from. If they are identical, the build is kept and the record simply brought
up to date — no rebuild. If they differ, it rebuilds, as before.

**The check also runs the other way, and that is the part you may actually have felt.** Some changes
to your code moved what gets compiled without moving anything the old check looked at — most
...
rebuilt even when its inputs are unchanged, because those files are addressed per release and mixing
them across releases is exactly how a portal can be brought down. The reasoning is written up in
