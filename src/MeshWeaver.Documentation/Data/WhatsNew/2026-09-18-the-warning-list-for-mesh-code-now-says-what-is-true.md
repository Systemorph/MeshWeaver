...
were hidden, but because no ordinary build in the platform reports them either, and the mesh check
had been the only compiler in the fleet still doing so. One family is about missing documentation
comments; the other is about the *version numbers on referenced libraries* not lining up exactly, a
property of which libraries are in the room and never of anything an author wrote.
...
available, not the weakest: with nothing listed, any warning at all from any of this code fails
immediately. The files themselves stay, because "there is no known debt" and "nobody checked" must
never look the same, and the check still refuses to pass unless it confirms it actually ran.

**What the lists say now is what was measured.** All 4 documentation examples and 27 of the 28
sample models compile, and every one of them compiles without a single warning. Each file records
that count, records that the 37 lines went because the warnings were retired rather than because
