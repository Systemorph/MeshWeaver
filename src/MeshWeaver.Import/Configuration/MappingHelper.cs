...
{
    public static PropertyInfo GetProperty<TSource, TProperty>(
        this Expression<Func<TSource, TProperty>> selector
    )
    {
        if (selector == null)
        ...
        )
        {
            ...
            return property.ReflectedType == typeof(TSource)
                ? property
                : typeof(TSource).GetProperty(property.Name);
        }
