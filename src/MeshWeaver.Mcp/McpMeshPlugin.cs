...
🚨 Carry the content through as you read it, INCLUDING its '$type' discriminator and its camelCase property names. Rebuilding content by hand (or round-tripping it through your own serializer) drops the discriminator, and content without one is stored fine and then materialises as nothing — the node keeps its whole document and every page of it renders empty. When in doubt read the shape from 'get @Node/Path/schema/' (the node or its NodeType): it names the '$type', the properties and their types, and each property's description — which is where a reference to another node is called out, and those must point at real nodes.")]
    ...
    [Description("Partial update of a single node. Only the keys present in 'fields' are changed; omitted keys preserve existing values. Patchable fields: name, description, icon, category, order, content, preRenderedHtml, mainNode — anything else (id, path, namespace, nodeType, version, created*/lastModified*) is REFUSED with an error naming it, so a field this tool cannot apply is never reported as patched; change those with Update. 'content' deep-merges (RFC 7396): the nested keys you send are updated, the ones you omit are kept, and a null member deletes just that key — so you can change a single content field (e.g. {\"content\":{\"logo\":\"…\"}}) without resending the rest. Setting the whole 'content' to null is rejected. Prefer this over Update for small edits like icon/name/category; for edits inside a long Markdown body or source file prefer edit_content.")]
