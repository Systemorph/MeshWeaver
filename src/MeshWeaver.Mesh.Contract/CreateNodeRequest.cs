...
public record CreateNodeRequest(MeshNode Node) : IRequest<CreateNodeResponse>
{
    ...
    /// Thread's first <c>AppendUserMessageRequest</c> — without a second client
    /// round-trip. The mesh hub posts <c>Argument</c> fire-and-forget to the new
    /// node's address with the original requester's AccessContext; the target hub
    /// processes it through its normal handler pipeline (including its own
    /// permission check).
    /// </summary>
