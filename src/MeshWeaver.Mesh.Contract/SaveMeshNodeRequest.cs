...
/// by an async handler on the per-node hub that calls
/// <see cref="MeshWeaver.Mesh.Services.IStorageService.SaveNode"/>. Used by
...
/// by an async handler on the per-node hub that calls
/// <see cref="MeshWeaver.Mesh.Services.IStorageService.DeleteNode"/>. The
