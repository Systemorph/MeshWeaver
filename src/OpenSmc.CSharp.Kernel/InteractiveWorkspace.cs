...
namespace OpenSmc.CSharp.Kernel
{
    internal class InteractiveWorkspace : Workspace
    {
        ...
        public InteractiveWorkspace(IMetadataReferencesFactory metadataReferencesFactory) 
            : base(MefHostServices.DefaultHost, WorkspaceKind.Interactive)
        {
            ...
            _referenceAssemblies = ResolveRefAssemblies(metadataReferencesFactory);
        ...
        private static IReadOnlyCollection<MetadataReference> ResolveRefAssemblies(IMetadataReferencesFactory metadataReferencesFactory)
        {
            ...
            foreach (var assemblyRef in Directory.EnumerateFiles(ResolveRefAssemblyPath(), "*.dll"))
            {
                try
                {
                    var resolved = metadataReferencesFactory.GetOrAdd(assemblyRef, MetadataReferenceProperties.Assembly);
                    assemblyRefs.Add(resolved.Value);
                }
        ...
        public async Task UpdateWorkspaceAsync(CSharpScriptState scriptState)
        {
            try
            {
