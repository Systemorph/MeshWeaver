...
namespace OpenSmc.DataSetReader.Excel.BinaryFormat
{
	...
	internal class XlsBiffSST : XlsBiffRecord
	{
		...
		{
			uint offset = (uint)readoffset + 8;
			uint last = (uint)readoffset + RecordSize;
			int lastcontinue = 0;
			uint count = UniqueCount;
			while (offset < last)
			{
				XlsFormattedUnicodeString str = new XlsFormattedUnicodeString(Bytes, offset);
				...
				if (offset + size > last)
				{
					...
					if (encoding == 0 && str.IsMultiByte)
					{
						...
						Buffer.BlockCopy(Bytes, (int)(contoffset + 5 + len), buff, (int)(last - offset + len + len), (int)postfix);
					...
					else if (encoding == 1 && str.IsMultiByte == false)
					{
						...
						Buffer.BlockCopy(tempbytes, 0, buff, (int)(last - offset), tempbytes.Length);
						Buffer.BlockCopy(Bytes, (int)(contoffset + 5 + len + len), buff, (int)(last - offset + len), (int)postfix);
