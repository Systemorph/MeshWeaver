    ...
    public class PercentageAttribute : ValidationAttribute
    {
        ...
        protected override ValidationResult IsValid(object value, ValidationContext validationContext)
        {
            ...
            if (!ValidationByType.TryGetValue(valueType, out var validation))
                return new ValidationResult($"The {validationContext.MemberName} field must have type from these: {SupportedTypesList}.", new[] { validationContext.MemberName });

            if (!validation(value, MinPercentage, MaxPercentage))
                return new ValidationResult($"The {validationContext.MemberName} field value should be in interval from {MinPercentage} to {MaxPercentage}.", new[] { validationContext.MemberName });

            return ValidationResult.Success;
        }

        private static readonly IReadOnlyDictionary<Type, Func<object, double, double, bool>> ValidationByType
            = new Dictionary<Type, Func<object, double, double, bool>>
              {
        ...
        private static readonly string SupportedTypesList = string.Join(", ", ValidationByType.Keys.Select(x => x.FullName));
