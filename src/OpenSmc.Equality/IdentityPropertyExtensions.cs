...
namespace OpenSmc.Equality
{
    ...
    {
        private static readonly CreatableObjectStore<Type, (PropertyInfo[] identityProperties, PropertyInfo[] keyProperties)> PropertyStore = new(GetIdentityPropertiesInner);

        private static (PropertyInfo[] identityProperties, PropertyInfo[] keyProperties) GetIdentityPropertiesInner(Type type)
        {
            var identityAndSimilarProperties = type.GetProperties()
                                                   ...
                                                   .Select(p => new { Prop = p, ipAttr = p.GetSingleCustomAttribute<IdentityPropertyAttribute>(), keyAttr = p.HasAttribute<SkipKeyForEqualityAttribute>() ? null : p.GetSingleCustomAttribute<KeyAttribute>() })
                                                   .Where(x => x.ipAttr != null || x.keyAttr != null)
                                                   .ToLookup(x => x.ipAttr?.GetType() ?? x.keyAttr?.GetType(), x => x.Prop);

            var ips = identityAndSimilarProperties[typeof(IdentityPropertyAttribute)].DistinctBy(x => x.Name).ToArray();
            var keys = identityAndSimilarProperties[typeof(KeyAttribute)].DistinctBy(x => x.Name).ToArray();
        ...
        public static bool HasIdentityProperties(this Type type, Func<PropertyInfo, bool> predicate = null)
        {
