...
{
    public ImmutableList<DataGridColumn> Columns { get; init; } = ImmutableList<DataGridColumn>.Empty;

    //public DataGridControl WithColumn<TProp>(Expression<Func<TGridItem, TProp>> expression,
...
{
    internal DataGridColumn Column { get; init; } = new();
    public PropertyColumnBuilder<TGridItem, TProperty> WithProperty(string property) => this with { Column = Column with{Property =property }};
    public PropertyColumnBuilder<TGridItem, TProperty> IsSortable(bool sortable = true) => this with {Column = Column with{Sortable = sortable}};
    public PropertyColumnBuilder<TGridItem, TProperty> WithFormat(string format) => this with{Column = Column with { Format = format }};
}
...
public record DataGridControlBuilder<T>
{
    ...
    public DataGridControlBuilder<T> WithColumn<TProp>(Expression<Func<T, TProp>> expression,
        Func<PropertyColumnBuilder<T, TProp>, PropertyColumnBuilder<T, TProp>> config) => this with
    {
        Columns = Columns.Add(config.Invoke(new PropertyColumnBuilder<T, TProp>()
            .WithProperty(((PropertyInfo)((MemberExpression)expression.Body).Member).Name)).Column)
    };

    public DataGridControl Build() => new()
    {
        Columns = Columns
    };
}
