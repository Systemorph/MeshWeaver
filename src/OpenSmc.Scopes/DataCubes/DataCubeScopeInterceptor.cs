    ...
    where TElement : class
{
    ...
    private static readonly IDictionary<string, DimensionProperty> IdentityDimensions =
        GetDimensionProperties(typeof(TIdentity));
    private static readonly IDictionary<string, DimensionProperty> ElementDimensions =
        GetDimensionProperties(typeof(TElement));
    private static readonly IDictionary<string, DimensionProperty> ScopeDimensions =
        GetDimensionProperties(typeof(TScope));
    private readonly IScopeRegistry scopeRegistry;

    // ReSharper disable once StaticMemberInGenericType
    protected static readonly AspectPredicate[] AspectPredicates =
    {
        x => x.DeclaringType.IsDataCubeInterface() || x.DeclaringType.IsEnumerableInterface()
    };
    ...
    public override void Intercept(IInvocation invocation)
    {
        ...
        switch (invocation.Method.Name)
        {
            ...
            case nameof(IDataCube.Filter):
                invocation.ReturnValue = ((IFilterable)invocation.Proxy).Filter(
                    ((string, object)[])invocation.Arguments.First()
                );
                return;
        }
    ...
    private IEnumerable<DimensionDescriptor> GetDimensionDescriptors(IInvocation invocation)
    {
        ...
        var identityDimensionDescriptors = TuplesUtils<TIdentity>.GetDimensionDescriptors(
            dimensions
        );
        var elementDimensionDescriptors = TuplesUtils<TElement>.GetDimensionDescriptors(dimensions);
        var dimensionDescriptors = identityDimensionDescriptors
            .Concat(elementDimensionDescriptors)
            .ToList();
        ...
        var orderedDimensionDescriptors =
            from dim in dimensions
            join descriptor in dimensionDescriptors on dim equals descriptor.SystemName
            select descriptor;
        ...
        if (isByRow)
        {
            return new[]
            {
                new DimensionDescriptor(
                    DataCubeScopesExtensions.PropertyDimension,
                    typeof(PropertyInfo)
                )
    ...
    {
        foreach (
            var enumerable in DataCubeProperties.Select(p =>
                (IEnumerable<TElement>)p.Reflector.GetValue(invocation.Proxy)
            )
        )
        {
            if (enumerable != null)
    ...
    protected object GetSlices(IInvocation invocation)
    {
        ...
        if (myIdentityDimensions.Any())
        {
            ...
            var identityTuple = new DimensionTuple(
                myIdentityDimensions.Select(x =>
                    (x.DimensionSystemName, x.Reflector.GetValue(identity))
                )
            );
        ...
        if (typeof(TScope) != typeof(TElement) || DataElementProperties.Length == 0)
        {
            ...
            tuple = tuple.Enrich(
                myScopeDimensions.Select(x =>
                    (x.DimensionSystemName, x.Reflector.GetValue(invocation.Proxy))
                )
            );
        }
        ...
        var dataCubeSlices = DataCubeProperties
            ...
            .SelectMany(c =>
            {
                var propertyTuple = tuple.Enrich(
                    (DataCubeScopesExtensions.PropertyDimension, c.Property)
                );
                return c
                    .DataCube.GetSlices(dimensions)
        ...
        var elementSlices = DataElementProperties
            .Select(p => new
            {
            ...
            {
                var propertyTuple = tuple.Enrich(
                    (DataCubeScopesExtensions.PropertyDimension, (object)c.Property)
                        .RepeatOnce()
                        .Concat(
                            elementDimensionProperties.Select(dp =>
                                (dp.DimensionSystemName, dp.Reflector.GetValue(c.Element))
                            )
                        )
                );
    ...
    private static DimensionProperty[] GetDimensions(
        string[] dimensions,
        IDictionary<string, DimensionProperty> dimensionProperties
    )
    {
        var myIdentityDimensions = dimensions
            .Select(d => dimensionProperties.TryGetValue(d, out var dp) ? dp : default)
            .Where(x => x != default)
            .ToArray();
    ...
    {
        var identityDimensions = type.GetProperties()
            .Select(p => new
            {
                Property = p,
                DimensionAttribute = p.GetCustomAttribute<DimensionAttribute>()
            })
            .Where(p => p.DimensionAttribute != null)
            .Select(p => new DimensionProperty(
                p.DimensionAttribute.Name,
                p.DimensionAttribute.Type,
                p.Property
            ))
