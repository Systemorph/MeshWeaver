...
public class LocalSourceContentVersionTest(ITestOutputHelper output) : MonolithMeshTestBase(output)
{
    ...
    public async Task EqualLengthContentEdits_ChangeTheListedVersion(string fileName)
    {
        ...
        try
        {
            ...
            var first = (await source.ListPackages("HEAD").Should().Within(TestTimeouts.Quick).Emit(cancellationToken: TestContext.Current.CancellationToken)).Single();
            var unchanged = (await source.ListPackages("HEAD").Should().Within(TestTimeouts.Quick).Emit(cancellationToken: TestContext.Current.CancellationToken)).Single();
            ...
            var edited = (await source.ListPackages("HEAD").Should().Within(TestTimeouts.Quick).Emit(cancellationToken: TestContext.Current.CancellationToken)).Single();
            ...
            var infrastructureOnly = (await source.ListPackages("HEAD").Should().Within(TestTimeouts.Quick).Emit(cancellationToken: TestContext.Current.CancellationToken)).Single();
