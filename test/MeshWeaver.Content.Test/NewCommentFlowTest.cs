...
public class NewCommentFlowTest(ITestOutputHelper output) : MonolithMeshTestBase(output)
{
    ...
    // NOTE: the suggest-edit test that used to live here asserted the OPPOSITE of today's contract —
    // it pinned "a suggestion creates a _Tracking satellite and leaves the document alone". Suggested
    // edits are now applied as normal versioned writes and the tracked-change view is PROJECTED from
    // the version history (ChangeProjection), so nothing is stored. The replacement coverage is
    // TrackedChangeProjectionTest (a document edited N times yields N tracked entries with the right
    // authors and NO _Tracking node) plus ChangeProjectionTest for the pure projection.
}
