...
public class FutuReAnalysisTest(ITestOutputHelper output) : MonolithMeshTestBase(output)
{
    ...
    {
        // AsiaRe is often the FIRST FutuRe BU compiled in an isolated shard, so it eats
        // the full cold Roslyn compile (45-55s on slow CI — genuine work, not a perf bug:
        // the BusinessUnit NodeType is compiled by Roslyn on first activation). The 100s
        // settle wait + the 120s [Fact] cap accommodate it; the cap still bounds a hang.
    ...
    /// </para>
    /// </summary>
    private async Task<StackControl> GetSettledOverview(string addressPath, int seconds = 100)
    {
