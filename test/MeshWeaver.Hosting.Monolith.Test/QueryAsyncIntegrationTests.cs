...
public class QueryAsyncIntegrationTests(ITestOutputHelper output) : MonolithMeshTestBase(output)
{
    ...
    {
        var p = P();
        await NodeFactory.CreateNodeAsync(MeshNode.FromPath($"{p}/Alice") with { Name = "Alice Chen", NodeType = "User", Content = new { name = "Alice Chen" } });
        await NodeFactory.CreateNodeAsync(MeshNode.FromPath($"{p}/Bob") with { Name = "Bob Wilson", NodeType = "User", Content = new { name = "Bob Wilson" } });
        await NodeFactory.CreateNodeAsync(MeshNode.FromPath($"{p}/Carol") with { Name = "Carol Martinez", NodeType = "User" });
        await NodeFactory.CreateNodeAsync(MeshNode.FromPath($"{p}/Public_Access") with { Name = "Public Access", NodeType = "AccessAssignment" });

        var results = await MeshQuery.QueryAsync<MeshNode>($"nodeType:User namespace:{p} scope:descendants").ToListAsync();
    ...
    {
        var p = P();
        await NodeFactory.CreateNodeAsync(MeshNode.FromPath($"{p}/Alice") with { Name = "Alice Chen", NodeType = "User" });
        await NodeFactory.CreateNodeAsync(MeshNode.FromPath($"{p}/Bob") with { Name = "Bob Wilson", NodeType = "User" });

        var results = await MeshQuery.QueryAsync<MeshNode>($"nodeType:User namespace:{p}").ToListAsync();
