...
public class PartitionAccessTest(ITestOutputHelper output) : MonolithMeshTestBase(output)
{
        ...
        => ConfigureMeshBase(builder)
            ...
            .AddMeshNodes(
                ...
                new MeshNode("TestPartition", PartitionNodeType.Namespace)
                {
                    ...
                    {
                        BasePaths = new HashSet<string> { "TestOrg" },
    ...
    [Fact(Timeout = 10000)]
    public async Task PartitionNode_HasCorrectBasePaths()
    {
        var results = await MeshQuery.QueryAsync<MeshNode>(
        ...
        def.BasePaths.Should().Contain("TestOrg");
        def.StorageType.Should().Be("InMemory");
        def.Description.Should().Contain("Test partition");
    }

    [Fact(Timeout = 10000)]
    public async Task DocumentationPartition_HasDocBasePath()
    {
        var results = await MeshQuery.QueryAsync<MeshNode>(
    ...
    public async Task OrganizationCreation_CreatesPartitionNode()
    {
        ...
        partitionDef!.BasePaths.Should().Contain("Globex");
    }
}
