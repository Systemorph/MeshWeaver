...
public class UserAccessTests(ITestOutputHelper output) : MonolithMeshTestBase(output)
{
    ...
    public async Task AddUserRole_GlobalRole_GrantsPermissionsEverywhere()
    {
        ...
        var permissions = await securityService.GetEffectivePermissionsAsync("ACME/Software/SomeProject", "NewUser", TestTimeout);
    ...
    public async Task GetEffectivePermissions_GlobalAdmin_HasAllPermissionsEverywhere()
    {
        ...
        var permDeep = await securityService.GetEffectivePermissionsAsync("ACME/Software/ProductLaunch/Todo/Task1", "Roland", TestTimeout);
    ...
    {
        var securityService = Mesh.ServiceProvider.GetRequiredService<ISecurityService>();
        await securityService.AddUserRoleAsync("Alice", "Editor", "ACME/Software", "system", TestTimeout);

        var permACME = await securityService.GetEffectivePermissionsAsync("ACME/Software", "Alice", TestTimeout);
        var permChild = await securityService.GetEffectivePermissionsAsync("ACME/Software/ProductLaunch", "Alice", TestTimeout);
