...
public class OrleansUserOwnedModelTest(ITestOutputHelper output) : OrleansSharedTestBase(output)
{
    ...
    [Fact(Timeout = 60_000, Skip = "Routing layer is now robust: config-driven StreamRoutedAddressTypes (4d72ff1ab predecessor), parent-chain walk in RouteStreamMessage, HostedHubsCollection.HubAdded subject — all unit-tested in MeshWeaver.Data.Test.RouteStreamMessageParentWalkTest. But the rotate flow still times out at cache.Update — likely cause: the sync sub-hub is created on a DIFFERENT process (silo vs client) than the one receiving the DataChangedEvent, OR there's a race even on the parent chain. Next investigation: capture the silo's MESHWEAVER_MSG_TRACE for cache/sync hubs, confirm which process hosts which sync sub-hub, then either subscribe to HubAdded for race-defense or route via a Subject-backed buffer.")]
