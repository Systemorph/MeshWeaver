...
public class GhostSchemaInvariantTests
{
    ...
    [Fact(Timeout = 60000)]
    public void WriteToUnprovisionedPartition_CreatesNoSchema_AndDoesNotPersist()
    {
        ...
        provider.Adapter.Write(node, JsonSerializerOptions.Default)
            .Catch((Exception _) => Observable.Return<MeshNode?>(null))
            .Should().Within(30.Seconds()).Emit();

        // 🚨 The invariant: no ghost schema was conjured.
        SchemaCount(ghost, ct).Should().Within(30.Seconds()).Be(0L);

        // And nothing persisted: a read tolerates the absent schema and finds nothing.
        var read = provider.Adapter.Read($"{ghost}/Foo", JsonSerializerOptions.Default)
            .Should().Within(30.Seconds()).Emit();
        Assert.Null(read);
    }

    [Fact(Timeout = 60000)]
    public void EnsurePartitionProvisioned_IsTheOneSchemaCreationPath_AndIsIdempotent()
    {
        var ct = TestContext.Current.CancellationToken;
        var provider = NewProvider();
        ...
        try
        {
            ...
            SchemaCount(part, ct).Should().Within(30.Seconds()).Be(0L);

            // The ONE schema-creation path (what OwnsPartitionProvisioningValidator calls).
            provider.EnsurePartitionProvisioned(part).Should().Within(60.Seconds()).Emit();
            SchemaCount(part, ct).Should().Within(30.Seconds()).Be(1L);

            // Idempotent: the promise-cache replays; no second CREATE SCHEMA, no error.
            provider.EnsurePartitionProvisioned(part).Should().Within(30.Seconds()).Emit();
            SchemaCount(part, ct).Should().Within(30.Seconds()).Be(1L);
            ...
            provider.Adapter.Write(node, JsonSerializerOptions.Default)
                .Should().Within(30.Seconds()).Emit();
            _fixture.DataSource.ScalarLong($"SELECT COUNT(*) FROM \"{part}\".mesh_nodes", ct)
                .Should().Within(30.Seconds()).Be(1L);
        }
        finally
        {
            _fixture.DataSource.ExecuteNonQuery($"DROP SCHEMA IF EXISTS \"{part}\" CASCADE", ct)
                .Should().Within(30.Seconds()).Emit();
        }
    }
}
