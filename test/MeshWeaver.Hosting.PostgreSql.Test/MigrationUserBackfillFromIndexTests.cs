...
public class MigrationUserBackfillFromIndexTests
{
    ...
    [Fact(Timeout = 60000)]
    public async Task V05Backfill_SourcesUsersFromMatview_WritesSelfGrantIntoUserPartition_NoUserSchema()
    {
        ...
        await _fixture.DataSource.ExecuteNonQuery($"SELECT public.ensure_partition_schema('{user}')", ct)
        ...
        await _fixture.DataSource.ExecuteNonQuery(V05BackfillSql, ct).Should().Within(30.Seconds()).Emit();
