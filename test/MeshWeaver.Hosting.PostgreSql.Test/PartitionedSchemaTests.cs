...
public class PartitionedSchemaTests
{
    ...
    public async Task CreateStore_CreatesSchemaTables()
    {
        ...
        var store = await factory.CreateStoreAsync("TestDomain");
        ...
        await using var cmd2 = _fixture.DataSource.CreateCommand(
            ...
            """);
        var table = await cmd2.ExecuteScalarAsync();
    ...
    public async Task SaveAndRead_AcrossSchemas_DataIsolated()
    {
        ...
        var storeA = await factory.CreateStoreAsync("Alpha");
        var storeB = await factory.CreateStoreAsync("Beta");
        ...
        var readA = await storeA.PersistenceCore.GetNodeAsync("Alpha/Reports", _options);
        readA.Should().NotBeNull();
        readA!.Name.Should().Be("Alpha Reports");

        // Alpha should not have Beta data
        var readB = await storeA.PersistenceCore.GetNodeAsync("Beta/Reports", _options);
        readB.Should().BeNull("Beta data should not be in Alpha schema");

        // Read back from Beta
        var readBeta = await storeB.PersistenceCore.GetNodeAsync("Beta/Reports", _options);
    ...
    public async Task RoutingCore_SaveRoutes_ByFirstSegment()
    {
        ...
        var readA = await router.GetNodeAsync("Gamma/Doc1", _options);
        readA.Should().NotBeNull();
        readA!.Name.Should().Be("Gamma Doc");

        var readB = await router.GetNodeAsync("Delta/Doc1", _options);
    ...
    public async Task DiscoverPartitions_FindsExistingSchemas()
    {
        ...
        await factory.CreateStoreAsync("Discover1");
        await factory.CreateStoreAsync("Discover2");
    ...
    public async Task Delete_InOnePartition_DoesNotAffectOther()
    {
        ...
        var readA = await router.GetNodeAsync("Eta/Item1", _options);
        readA.Should().BeNull();

        // Theta item should still exist
        var readB = await router.GetNodeAsync("Theta/Item1", _options);
        readB.Should().NotBeNull();
