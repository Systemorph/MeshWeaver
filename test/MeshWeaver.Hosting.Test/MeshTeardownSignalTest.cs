...
public class MeshTeardownSignalTest
{
    ...
    public void TeardownReport_is_clean_only_when_nothing_survived()
    {
        ...
        new TeardownReport(0, false).Clean.Should().BeFalse(
            "the second term is binary-compat residue (#2442) and is always true in production, "
            + "but while the field exists Clean must still honour it — a report that ignored one of "
            + "its own inputs would be the next silent green");
    }
}
