...
public class PathResolutionCachePoisonTest(ITestOutputHelper output) : HubTestBase(output)
{
    ...
    public async Task HungFirstQuery_DoesNotPoisonCache()
    {
        ...
        await Assert.ThrowsAsync<TimeoutException>(() =>
            svc.ResolvePath("A/B").Take(1).Timeout(TimeSpan.FromSeconds(2)).Await(TestContext.Current.CancellationToken));
        ...
        var resolution = await svc.ResolvePath("A/B")
            .Take(1).Timeout(TimeSpan.FromSeconds(5)).Await(TestContext.Current.CancellationToken);
    ...
    public async Task PositiveResolution_IsServedFromCache_OnSecondCall()
    {
        ...
        var first = await svc.ResolvePath("A/B").Take(1).Timeout(TimeSpan.FromSeconds(5)).Await(TestContext.Current.CancellationToken);
    ...
    public async Task UpdatedNode_KeepsRouteResolutionWarm_WhileNodeReadersRefresh()
    {
        ...
        var refreshed = await svc.ResolvePath("A/B").Take(1).Timeout(TimeSpan.FromSeconds(5)).Await(TestContext.Current.CancellationToken);
    ...
    public async Task CreatedAndDeleted_StillInvalidateRouteResolution()
    {
        ...
        var shallow = await svc.ResolveRoute("A/B/C").Take(1).Timeout(TimeSpan.FromSeconds(5)).Await(TestContext.Current.CancellationToken);
        Assert.Equal("A", shallow!.Prefix);

        // Created(A/B) deepens the resolution of A/B/C — the cached route entry must go.
        feed.Publish(MeshChangeEvent.Created(new MeshNode("B", "A") { NodeType = "Markdown" }));
        var deeper = await svc.ResolveRoute("A/B/C").Take(1).Timeout(TimeSpan.FromSeconds(5)).Await(TestContext.Current.CancellationToken);
        ...
        var shallowAgain = await svc.ResolveRoute("A/B/C").Take(1).Timeout(TimeSpan.FromSeconds(5)).Await(TestContext.Current.CancellationToken);
