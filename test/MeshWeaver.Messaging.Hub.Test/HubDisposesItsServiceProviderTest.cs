...
public class HubDisposesItsServiceProviderTest(ITestOutputHelper output) : HubTestBase(output)
{
    ...
    public async Task DisposingTheParent_ClosesAHostedHubsScopeToo()
    {
        ...
        host.Dispose();
        await host.DisposalCompleted.FirstOrDefaultAsync().Await(TestContext.Current.CancellationToken).WaitAsync(TimeSpan.FromSeconds(120), TestContext.Current.CancellationToken);
