...
public class FileSystemPersistenceTest(ITestOutputHelper output) : MonolithMeshTestBase(output)
{
    ...
    public async Task Read_Exists_ReturnsCorrectResult()
    {
        ...
        // Assert
        (await _persistence.Exists("graph/org1").FirstAsync().ToTask()).Should().BeTrue();
        (await _persistence.Exists("graph/nonexistent").FirstAsync().ToTask()).Should().BeFalse();
    }
    ...
    public async Task Delete_SingleNode_RemovesFromFileSystem()
    {
        ...
        (await _persistence.Exists("graph/org1").FirstAsync().ToTask()).Should().BeFalse();
    }
