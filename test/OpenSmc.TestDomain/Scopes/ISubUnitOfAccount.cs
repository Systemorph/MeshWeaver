...
namespace OpenSmc.TestDomain.Scopes
{
    ...
    {
        IUnitOfAccount UnitOfAccount => GetScope<IUnitOfAccount>(new DataNode { SystemName = Identity.Parent });
    ...
    public interface IUnitOfAccount : IScope<DataNode, UnitOfAccountStorage>
    {
        ISubUnitOfAccount SubUnits =>
            GetScope<ISubUnitOfAccount>(Identity,
                     o => o.WithFactory(() => GetStorage()
                           .GetIdentitiesByParent(Identity.SystemName)
                           .Select(GetScope<ISubUnitOfAccount>)
                           .Aggregate()));

        IUnitOfAccountContractualServiceMargin ContractualServiceMargin => GetScope<IUnitOfAccountContractualServiceMargin>();
    }
    ...
    public interface IFxConversion : IScope<DataNode, UnitOfAccountStorage>
    {
        static ApplicabilityBuilder FxConversionApplicability(ApplicabilityBuilder builder) =>
            builder.ForScope<IFxConversion>(s =>
                                                s.WithApplicability<IFunctionalFxConversion>(x => x.Identity.FxType == FxType.Functional, p => p.ForMember(x => x.FxMid))
                                                 .WithApplicability<IGroupFxConversion>(x => x.Identity.FxType == FxType.Group || x.Identity.FxType == FxType.Functional));
    ...
    public interface IBestEstimate : IFxConversion
    {
        ...
        static ApplicabilityBuilder Applicability(ApplicabilityBuilder builder) =>
            builder
                .ForScope<IBestEstimate>(
                                         s => s
                                              .WithApplicability<IBestEstimateMethod1>
                                                  (
    ...
    public interface IUnitOfAccountContractualServiceMargin : IScope<DataNode, UnitOfAccountStorage>
    {
        static ApplicabilityBuilder Applicability(ApplicabilityBuilder builder) =>
            builder
                .ForScope<IUnitOfAccountContractualServiceMargin>
                    (s => s
                          .WithApplicability<IProfitableContractualServiceMargin>(
                                                                                  x => x.IsProfitable,
        ...
        double Delta => ParentScope.SubUnits.BestEstimate.Delta + ParentScope.SubUnits.RiskAdjustment.Delta;
    ...
    {
        double AmortizationFactor => 0.1;
        double IUnitOfAccountContractualServiceMargin.LossComponentReversal => -EoPTemp * AmortizationFactor;
    }
    ...
    public interface IContractualServiceMargin : IScope<DataNode, UnitOfAccountStorage>
    {
        ...
        double Delta => ParentScope.UnitOfAccount.ContractualServiceMargin.Delta * Weight / ParentScope.UnitOfAccount.SubUnits.ContractualServiceMargin.Weight;

        double Amortization => ParentScope.UnitOfAccount.ContractualServiceMargin.Amortization * Weight / ParentScope.UnitOfAccount.SubUnits.ContractualServiceMargin.Weight;
        double LossComponentReversal => ParentScope.UnitOfAccount.ContractualServiceMargin.LossComponentReversal * Weight / ParentScope.UnitOfAccount.SubUnits.ContractualServiceMargin.Weight;
    ...
    public class UnitOfAccountStorage
    {
        ...
        {
            Identities = Enumerable.Range(1, nScopes).Select(i => new DataNode
                                                                  {
                                                                      SystemName = i.ToString(),
                                                                      Parent = parent(i).ToString(),
                                                                      Method = parent(i)
                                                                  }).ToArray();
            identitiesByParent = Identities.GroupBy(x => x.Parent).ToDictionary(x => x.Key, x => x.ToArray());
        }
        ...
        private static readonly Dictionary<FxTime, double> FxRates = Enum.GetValues(typeof(FxTime)).Cast<FxTime>().ToDictionary(x => x, _ => Random.NextDouble());
