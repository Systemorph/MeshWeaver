...
namespace OpenSmc.TestDomain.SimpleData
{
    ...
    public static class SimpleAccountingFactory
    {
        ...
        {
            return Enumerable.Range(0, n).Select(i => new SimpleAccounting { Delta = i, SystemName = $"i{i + 1}", DisplayName = $"Item {i + 1}" }).ToArray();
        }

        public static TRecord[] GetData<TRecord>(int n)
            where TRecord : SimpleAccounting, new()
        {
            return Enumerable.Range(0, n).Select(i => new TRecord { Delta = i, SystemName = $"i{i + 1}", DisplayName = $"Item {i + 1}" }).ToArray();
        }
    }
}