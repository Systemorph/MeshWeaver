...
public static class ProjectFile
{
        ...
        string projectPath, IReadOnlyCollection<string> accepted, IReadOnlyDictionary<string, string> globals)
    {
        ...
        /// <summary>
        /// One SDK default item glob — <c>**/&lt;pattern&gt;</c> under the project, minus the output
        /// trees, the dot-directories and <c>$(DefaultItemExcludes)</c>. Shared by the default
        /// Compile items (<c>*.cs</c>) and the default EmbeddedResource items (<c>*.resx</c>), which
        /// the SDK declares with the same exclusion set.
        /// </summary>
        /// <param name="pattern">The file pattern, e.g. <c>*.resx</c>.</param>
        /// <returns>Absolute paths, in enumeration order.</returns>
        private IEnumerable<string> DefaultGlob(string pattern)
        {
            var excludes = Split(Prop("DefaultItemExcludes"))
                .Select(p => Path.GetFullPath(Path.Combine(ProjectDirectory, p.TrimEnd('*', '/', '\\'))))
                .ToArray();
            foreach (var file in Directory.EnumerateFiles(ProjectDirectory, pattern, SearchOption.AllDirectories))
            {
